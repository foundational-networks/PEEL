...
const EtherMacBase::EtherDescr EtherMacBase::etherDescrs[NUM_OF_ETHERDESCRS] = {
    {
        ONE_POINT_FIVE_MEGABIT_ETHERNET_TXRATE,
        0.5 / ONE_POINT_FIVE_MEGABIT_ETHERNET_TXRATE,
        0,
        B(0),
    ...
    {
        FIFTY_GIGABIT_ETHERNET_TXRATE,
        0.5 / FIFTY_GIGABIT_ETHERNET_TXRATE,
        0,
        B(0),
        B(-1),    // half-duplex is not supported
        B(0),
...
simsignal_t EtherMacBase::packetHopCountSignal = registerSignal("packetHopCount");
...
EtherMacBase::~EtherMacBase()
{
    recordScalar("perPacketFabricDelaySum", per_packet_fabric_delay_sum);
    recordScalar("perPacketFabricDelayNum", per_packet_fabric_delay_num);
...
void EtherMacBase::calculate_fabric_delay(simtime_t time_sent_from_source_host) {
    // per packet fabric delay
    double fabric_delay = (simTime() - time_sent_from_source_host).dbl();
    if (fabric_delay <= 0)
        throw cRuntimeError("fabric_delay <= 0");
//    std::cout << "time: " << simTime() << " time sent is " << time_sent_from_source_host << " so the fabric delay is " << fabric_delay << endl;
    per_packet_fabric_delay_sum += fabric_delay;
    per_packet_fabric_delay_num++;
}
...
//    if (use_host_assisted_routing && use_udp) {
    if (use_udp) {
        auto parent_module = getParentModule()->getParentModule();
        std::string parent_name = parent_module->getName();
//        std::cout << getFullPath() << ": " << frame->getDst_idx_listArraySize() << endl;
        // if we're not using network_assisted then getDst_idx_listArraySize should be empty
        if (parent_name.compare("server") == 0) {
            int parent_index = parent_module->getIndex();
            for (int i = 0; i < frame->getDst_idx_listArraySize(); i++) {
//                std::cout << "Index: " << frame->getDst_idx_list(i) << endl;
                if (frame->getDst_idx_list(i) == parent_index) {
                    EV << "Server found as a destination for this packet" << endl;
                    return false;
                }
            }
        }
