...
class INET_API ApplicationBase : public OperationalBase
{
  public:
    ...
    unsigned long long nvlink_traversal_cnt = 0;
  ...
  protected:
    ...
    virtual int generate_next_batch(unsigned long flow_id, int gpu_idx=-1) {throw cRuntimeError("generate_next_batch not impelmented but called!");}
};
