...
class INET_API Ipv4 : public OperationalBase, public NetfilterBase, public INetworkProtocol, public IProtocolRegistrationListener, public cListener
{
  ...
  protected:
    ...
    int read_src_gpu_idx(Packet *packet);
...
//    std::list<unsigned long> flow_lru_tracker;
    std::map<simtime_t, unsigned long> flow_lru_tracker;   // Tracks the lru for flows: last_updated --> hash_of_flows
    // we assumed that no two flows are updated at the same time, so the update time in each server is unique :)
    int marking_type;
    simtime_t delta; //time gap for differentiating flow lets
    std::string remove_length_from_name(std::string name);
...
//    std::list<unsigned long> ordering_component_flow_lru_tracker;
    std::map<simtime_t, unsigned long> ordering_component_flow_lru_tracker;   // Tracks the lru for flows: last_updated --> hash_of_flows
    // we assumed that no two flows are updated at the same time, so the update time in each server is unique :)
    int ordering_component_flow_hash_table_size = -1;
    bool should_check_ordering(std::string packet_name);
    void print_marking_info(MarkingInfoHolder marking_info_holder);
    ...
    void push_old_stored_packets(std::unordered_map<unsigned long,
            OrderingComponentFlowInfo*>::iterator flow_it,
            bool should_print = false, bool restart_timer = false);
    ...
    bool use_udp = false;
//    bool use_host_assisted_routing;
