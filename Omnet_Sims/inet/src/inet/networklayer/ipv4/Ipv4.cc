...
void Ipv4::initialize(int stage)
{
    ...
    if (stage == INITSTAGE_LOCAL) {
        ...
        // swift
        use_swift = par("use_swift");

        use_udp = par("use_udp");
//        use_host_assisted_routing = par("use_host_assisted_routing");
    }
}
...
void Ipv4::handleIncomingDatagram(Packet *packet)
{
    ...
    // bypass for bolt src packets
    if (packet_name.find("SRC") == std::string::npos && !verifyCrc(ipv4Header)) {
        throw cRuntimeError("Ipv4: here for the sake of bolt -- ipv4: CRC error found, drop packet");
    ...
    if (ipv4Header->getTotalLengthField() > packet->getDataLength()) {
        std::cout << "length error found, sending ICMP_PARAMETER_PROBLEM\n";
        std::cout << "ipv4Header->getTotalLengthField(): " << ipv4Header->getTotalLengthField() << endl;
        std::cout << "packet->getDataLength(): " << packet->getDataLength() << endl;
...
void Ipv4::preroutingFinish(Packet *packet)
{
    ...
    if (fromIE->isLoopback()) {
        ...
        if (rt->isLocalAddress(destAddr) || fromIE->getProtocolData<Ipv4InterfaceData>()->getIPAddress().isUnspecified()) {
            reassembleAndDeliver(packet);
//        } else if (use_udp && use_host_assisted_routing) {
        } else if (use_udp) {
            // this packet is already checked by mac layer
            // for now we removed use_host_assisted_routing
            EV << "use_udp && use_host_assisted_routing: packet is for this server" << endl;
            reassembleAndDeliver(packet);
...
void Ipv4::assign_payload_info_to_packet(Packet *main_packet) {
    EV << "assign_payload_info_to_packet: " << main_packet->str() << endl;
    auto packet = main_packet->dup();
    if (use_udp)
        packet->removeAtFront<UdpHeader>();
    else
        packet->removeAtFront<tcp::TcpHeader>();
...
void Ipv4::handlePacketFromHL(Packet *packet)
{
    ...
    if (use_udp) {
        EV << "Adding outgoing index and dest addr index" << endl;
        const auto& ipv4Header = packet->peekAtFront<Ipv4Header>();
        Ipv4Address destAddr = ipv4Header->getDestAddress();
        if (packet->findTag<InterfaceReq>() == nullptr) {
            if (packet->findTag<NextHopAddressReq>() != nullptr)
                throw cRuntimeError("We don't have InterfaceReq but have NextHopAddressReq");

            const Ipv4Route *re = rt->findBestMatchingRoute(destAddr);
            if (re) {
                // todo: for now we hardcode the base id to 101
                // 100 is for loopback so base id for eth0 is 101
//                int interface_id = re->getInterface()->getInterfaceId();
                int interface_id = 101;

                interface_id += read_src_gpu_idx(packet->dup());
                packet->addTagIfAbsent<InterfaceReq>()->setInterfaceId(interface_id);
                // Sepehr: For gpu tests, we changed this to avoid longest prefix matching
...
{
    EV << "datagramLocalOut: " << packet->str() << endl;
...
int Ipv4::read_src_gpu_idx(Packet *packet) {
    int gpu_idx = 0;
    packet->setFrontIteratorPosition(b(0));
...
//
    int payload_gpu_idx = payload->getSrc_gpu_idx();
    if (payload_gpu_idx >= 0)
        gpu_idx = payload_gpu_idx;
...
void Ipv4::routeUnicastPacket(Packet *packet)
{
    ...
    const auto& ipv4Header = packet->peekAtFront<Ipv4Header>();
    Ipv4Address destAddr = ipv4Header->getDestAddress();
    EV_INFO << "Routing " << packet << " with destination = " << destAddr << endl;
    ...
    if (destIE) {
        ...
        if (re) {
            ...
            int interface_id = destIE->getInterfaceId();
            packet->addTagIfAbsent<InterfaceReq>()->setInterfaceId(interface_id);
            packet->addTagIfAbsent<NextHopAddressReq>()->setNextHopAddress(re->getGateway());
        }
    }
...
//    bool is_tcp_segment = packet_name.find("tcpseg") != std::string::npos;
    bool is_arp_related = packet_name.find("arp") != std::string::npos;
    return should_use_v2_marking && has_ordering_layer && (!is_arp_related);
}

void Ipv4::print_marking_info(MarkingInfoHolder marking_info_holder) {
    EV << "Seq: " << marking_info_holder.seq << ", ret_count: " << marking_info_holder.ret_count <<
            ", flow_let_id: " << marking_info_holder.flow_let_id << ", is_first_packet: " <<
            marking_info_holder.is_first_packet << endl;
    EV << "----------------------------------------" << endl;
...
bool Ipv4::compare_packets(Packet* first, Packet* second) {
    ...
    std::string first_packet_name = remove_length_from_name(first_dup->getName());
    ...
    std::string second_packet_name = remove_length_from_name(second_dup->getName());
...
bool Ipv4::apply_LAS_ordering(Packet *packet, unsigned long hash_of_flow,
    ...
    print_marking_info(marking_info_holder);
    ...
    if (flow_found != ordering_component_flow_hash_table.end()) {
        ...
        if (marking_info_holder.flow_let_id > flow_found->second->expected_flow_let_id) {
            ...
            //todo: If anything is left in the queue, for now push them up.
            if (flow_found->second->stored_packet_hash_table.size() != 0) {
//                std::cout << "Changing flow id when some packets are stored! incident #" << test_counter <<
//                                        ", there are " << (flow_found->second->stored_packet_hash_table.size()) <<
//                                        " packets stored!" << endl;
//                test_counter++;
                for (std::map<unsigned long, OrderingComponentPacketInfo*>::iterator it =
                                    flow_found->second->stored_packet_hash_table.begin();
...
void Ipv4::push_old_stored_packets(std::unordered_map<unsigned long, OrderingComponentFlowInfo*>::iterator flow_it,
        bool should_print, bool restart_timer) {
    if (marking_type == MARKING_SRPT) {
        ...
        while (flow_it->second->descending_stored_packet_hash_table.begin()->first >
            ...
            if (should_print)
                std::cout << "Pushing packet with seq=" << first_element_seq << " up because it's old and stored!" << endl;
...
//    std::cout << simTime() << endl;
    ...
    bool is_ndp_packet = eth_header->getIs_v2_dropped_packet_header();
    ...
    if (marking_info_holder.is_control_message) {
        // push the packet up
        if (should_print)
            std::cout << "Message is a control message: " << packet->str() << endl;
    ...
    if (should_print) {
        std::cout << "------------------------------------------" << endl;
        std::cout << "t=" << simTime() << "s, Packet received in ordering component: " << packet->str() << endl;
        std::cout << "Seq: " << marking_info_holder.seq << ", ret_count: " << marking_info_holder.ret_count <<
                        ", flow_let_id: " << marking_info_holder.flow_let_id << ", is_first_packet: " <<
                        marking_info_holder.is_first_packet << endl;
    }
    print_marking_info(marking_info_holder);
    ...
    if (flow_found != ordering_component_flow_hash_table.end()) {
        // Flow found
        if (should_print)
            std::cout << "Flow with hash " << hash_of_flow << " found. Expected seq is " << flow_found->second->expected_seq << endl;
        ...
        if (marking_info_holder.flow_let_id > flow_found->second->expected_flow_let_id) {
            ...
            //todo: If anything is left in the queue, for now push them up.
            if (should_print)
                std::cout << "marking_info_holder.flow_let_id > flow_found->second->expected_flow_let_id, packet of the next flowlet is received" << endl;
...
//                std::cout << "Changing flow id when some packets are stored! incident #" << test_counter <<
//                        ", there are " << (flow_found->second->descending_stored_packet_hash_table.size()) <<
//                        " packets stored!" << endl;
//                test_counter++;
                for (std::map<unsigned long, OrderingComponentPacketInfo*>::iterator it =
                                    flow_found->second->descending_stored_packet_hash_table.begin();
...
//                std::cout << "Changing the flow id while there are some packets in the storage" << endl;
            }
            ...
            if (marking_info_holder.is_first_packet) {
                ...
                if (should_print)
                    std::cout << "First packet with seq=" <<
                                            marking_info_holder.seq << " received. So we push everything up and get ready for a new co-flow" << endl;
                // push the current packet up
                result = false;
                flow_found->second->expected_seq =
                ...
                if(should_print)
                    std::cout << "Packet is out of order" << endl;
            ...
            if (should_print)
                std::cout << "marking_info_holder.flow_let_id < flow_found->second->expected_flow_let_id, packet of old flowlet" << endl;
            ...
            if (should_print)
                std::cout << "marking_info_holder.flow_let_id == flow_found->second->expected_flow_let_id" << endl;
            ...
            if (marking_info_holder.seq > flow_found->second->expected_seq) {
                ...
                if (should_print)
                    std::cout << "marking_info_holder.seq > flow_found->second->expected_seq, the packet is old" << endl;
                ...
                if (should_print)
                    std::cout << "marking_info_holder.seq == flow_found->second->expected_seq" << endl;
                if(!is_ndp_packet)
                    received_correctly_pushed_counter++;
                ...
                if (flow_found->second->timeoutMsg == nullptr) {
                    ...
                    EV << "seq == flow_found->second->expected_seq and no timeout. pushing packet up." << endl;
                    if (should_print)
                        std::cout << "seq == flow_found->second->expected_seq and no timeout. pushing packet up." << endl;
                    if(!is_ndp_packet)
                        emit(v2PacketQueueingTimeSignal, 0);
                    result = false;
                    ...
                    while (packet_received_before != flow_found->second->sent_packet_seqs_payload_length.end()) {
                        ...
                        if (should_print)
                            std::cout << "Packet with seq=" << flow_found->second->expected_seq << " received before. increasing the "
                                    "expected seq to " << flow_found->second->expected_seq - packet_received_before->second << endl;
                    ...
                    // stop timer
                    if (should_print)
                        std::cout << "seq == flow_found->second->expected_seq and timeout. packet fills a gap." << endl;
                    ...
                    send(packet, "transportOut");
                    numLocalDeliver++;
                    push_old_stored_packets(flow_found, should_print, false);
                    ...
                    while (max_iteration_count > 0 && (expected_is_stored || expected_was_received)) {
                        ...
                        if (expected_is_stored) {
                            ...
                            if (should_print)
                                std::cout << "Continue pushing. Pushing packet with seq=" << packet_found->first << " up." << endl;
                            ...
                            // check if the expected seq was sent before
                            if (should_print)
                                std::cout << "Continue pushing. Packet with seq=" << flow_found->second->expected_seq << " was received before." << endl;
                ...
                // The packet is out of order
                if (should_print)
                    std::cout << "The received packet is out of order." << endl;

                EV << "The received packet is out of order." << endl;
                if (is_ndp_packet) {
                    // just store it as sent and send the ndp packet up
                    PayloadInfoHolder payload_info_holder = extract_payload_info_holder(packet);
                    flow_found->second->sent_packet_seqs_payload_length.insert(
                                        std::pair<unsigned long, unsigned int>(marking_info_holder.seq,
                                                payload_info_holder.payload_length.get()));
    ...
    //                        throw cRuntimeError("A different packet with same seq was received before the previous packet was deleted or pushed up");
        ...
        // Flow does not exist initiate it
        if (should_print)
            std::cout << "No flow with hash " << hash_of_flow << " found." << endl;
        EV << "No flow with hash " << hash_of_flow << " found." << endl;
        unsigned long expected_seq;
        if (marking_info_holder.is_first_packet) {
            if (should_print)
                std::cout << "The received packet is in-order." << endl;
        ...
        if (!marking_info_holder.is_first_packet && is_ndp_packet) {
            PayloadInfoHolder payload_info_holder = extract_payload_info_holder(packet);
...
bool Ipv4::is_packet_reordered(Packet *packet) {
    ...
    // todo: for now I hash src_ip, dst_ip, src_port, dst_port, tcp_seq_num
    auto packet_dup = packet->dup();
    packet_dup->setFrontIteratorPosition(b(0));
    packet_dup->removeAtFront<EthernetPhyHeader>();
    auto eth_header = packet_dup->removeAtFront<EthernetMacHeader>();
    ...
    std::string packet_name = remove_length_from_name(packet->getName());
...
void Ipv4::handle_timeout_v2_SRPT(unsigned long hash_of_flow, unsigned long packet_seq) {
    ...
    if (flow_found != ordering_component_flow_hash_table.end()) {
        ...
        while (it != flow_found->second->descending_stored_packet_hash_table.end() && it->first > packet_seq) {
            ...
            auto packet_dup = it->second->packet->dup();
            packet_dup->setFrontIteratorPosition(b(0));
            packet_dup->removeAtFront<EthernetPhyHeader>();
            bool is_ndp_packet = packet_dup->removeAtFront<EthernetMacHeader>()->getIs_v2_dropped_packet_header();
            delete packet_dup;

            if (is_ndp_packet)
                throw cRuntimeError("How is a ndp packet timedout!");
...
void Ipv4::reassembleAndDeliverFinish(Packet *packet)
{
    ...
    if (upperProtocols.find(protocol) != upperProtocols.end()) {
        ...
        // this is just to test if ordering has some bug or not
        // this code segment removes all ndp packets before getting to ordering
//        auto packet_dup = packet->dup();
//        packet_dup->setFrontIteratorPosition(b(0));
//        packet_dup->removeAtFront<EthernetPhyHeader>();
//        auto eth_header = packet_dup->removeAtFront<EthernetMacHeader>();
//        delete packet_dup;
//        if(eth_header->getIs_v2_dropped_packet_header()) {
//            delete packet;
//            return;
//        }
...
void Ipv4::fragmentAndSend(Packet *packet)
{
    ...
    if (use_udp) {
        // this is used for load balancing in the network
        // fragmentation puts wrong source and dst port in the fragments
        auto ipv4Header = packet->removeAtFront<Ipv4Header>();
        auto udpHeader = packet->peekAtFront<UdpHeader>();
        ipv4Header->setOriginal_src_port(udpHeader->getSourcePort());
        ipv4Header->setOriginal_dst_port(udpHeader->getDestinationPort());
    ...
    if (flow_found != flow_hash_table.end()) {
        ...
        if (packet_found != flow_found->second->packet_hash_table.end()) {
            ...
            if (now - flow_found->second->last_updated > delta) {
                ...
                // todo: we don't use MAX_FLOW_LET_ID for now
                num_timeouts_marking++;
                marking_info_holder.seq = 0;
                marking_info_holder.ret_count = 0;
                flow_found->second->ret_count = marking_info_holder.ret_count;
...
std::string Ipv4::remove_length_from_name(std::string name) {
//    size_t found = name.find("(l=");
//    if (found != std::string::npos)
//        return name.substr(0, found);
    // for now return the name itself
    return name;
}
...
void Ipv4::apply_marking(Packet *transportPacket) {
    ...
    std::string packet_name = remove_length_from_name(transportPacket->getName());
    ...
    bool should_print = false;
    if (should_print) {
        EV << "t=" << simTime() << "s, Sending packet " << transportPacket->str() << " in the marking component" << endl;
        EV << "Seq: " << marking_info_holder.seq << ", ret_count: " << marking_info_holder.ret_count <<
                ", flow_let_id: " << marking_info_holder.flow_let_id << ", is_first_packet: " <<
                marking_info_holder.is_first_packet << endl;
        EV << "----------------------------------------" << endl;
    ...
    // todo: Extra checks
//    if (flow_hash_table.size() != flow_lru_tracker.size())
//        throw cRuntimeError("Mismatch: flow_hash_table.size() != flow_lru_tracker.size()");
//
//    if (flow_hash_table.size() > flow_hash_table_size)
//        throw cRuntimeError("Mismatch: flow_hash_table.size() > flow_hash_table_size");
//
//    for (std::list<unsigned long>::iterator flow_it = flow_lru_tracker.begin();
//            flow_it != flow_lru_tracker.end(); ++flow_it) {
//        auto flow_found = flow_hash_table.find(*flow_it);
//
//        if (flow_found == flow_hash_table.end())
//            throw cRuntimeError("mismatch between flow hash table and flow lru table");
//
//        if (flow_found->second->packet_hash_table.size() != flow_found->second->packet_lru_tracker.size())
//            throw cRuntimeError("Mismatch: flow_found->second->packet_hash_table.size() != flow_found->second->packet_lru_tracker.size()");
//
//        if (flow_found->second->packet_hash_table.size() > packet_hash_table_size)
//            throw cRuntimeError("Mismatch: flow_found->second->packet_hash_table.size() > packet_hash_table_size");
//    }
...
//    }
    ...
    // swift
    if (use_swift) {
        EV << "Adding swift's header overhead" << endl;
        auto swift_options = new Ipv4OptionSwift();
        ipv4Header->addOption(swift_options);
        ipv4Header->addChunkLength(B(swift_options->getLength()));
    }
...
{
    EV << "Ipv4::sendDatagramToOutput" << endl;
