...
{
//    unsigned int DCTCPWindowEnd = state -> snd.una;
//    unsigned int DCTCPBytesAcked = 0;
//    unsigned int DCTCPBytesMarked = 0;
}
...
void DCTcp::receivedDataAck(uint32 firstSeqAcked)
{
    ...
    EV << "SOUGOL: This is snd_una: " << state->snd_una << endl;
    EV << "SOUGOL: This is firstSeqAcked " << firstSeqAcked << endl;
    EV << "SOUGOL: This is bytesAcked: " << BytesAcked << endl;
    state -> DCTCPBytesAcked += BytesAcked;
    EV << "SOUGOL: This is DCTCP bytesAcked: " << state->DCTCPBytesAcked << endl;
    ...
    if (state->dupacks >= DUPTHRESH) {    // DUPTHRESH = 3
        ...
        bool can_decrease = (state->snd_una > state->DCTCPWindowEnd);
        bool can_decrease_due_to_deflection;
        if (!state->def_rate_limit_update_cwnd_once_per_rtt) {
            can_decrease_due_to_deflection = true;
        } else if (state->use_one_decrease_threshold) {
            can_decrease_due_to_deflection = can_decrease;
        } else {
            can_decrease_due_to_deflection = (state->snd_una > state->deflection_DCTCPWindowEnd);
        }

        if (can_decrease_due_to_deflection && state->main_packet_deflected && state->def_rate_limiting_type != DEF_NO_LIMIT) {
            if (state->def_rate_limiting_type == DEF_FR_LIMIT) {
                reduce_rate_based_on_FR();
            } else if (state->def_rate_limiting_type == DEF_RTO_LIMIT) {
                reduce_rate_based_on_RTO();
            } else if (state->def_rate_limiting_type == DEF_REGULAR_REACTION_LIMIT) {
                reduce_rate_based_on_ECN();
            } else
                throw cRuntimeError("DCTCP: Don't recognize def_rate_limiting_type");
            state->deflection_DCTCPWindowEnd = state->snd_nxt;
            if (state->use_one_decrease_threshold)
                state->DCTCPWindowEnd = state->snd_nxt;
        } else if (state && state->ect && state->gotEce) {
            ...
            EV << "SOUGOL: Enter ECN evaluation." << endl;
            conn->tcpMain->dctcp_num_packets_marked++;
            EV << "SOUGOL: This is the bytesACK:" << BytesAcked << endl;
            EV << "SOUGOL: This is the ECN marked PKT before adding:" << state->DCTCPBytesMarked << endl;
            state -> DCTCPBytesMarked += BytesAcked;
            BytesAcked = 0;
            EV << "SOUGOL: This is the ECN marked PKT after adding:" << state->DCTCPBytesMarked << endl;
            EV << "SOUGOL: This is the bytesACK after = 0:" << BytesAcked << endl;
            EV <<"SOUGOL: This is firstSeqAck: " << firstSeqAcked << endl;
            EV << "SOUGOL: This is the DCTCP Window: " << state->DCTCPWindowEnd << endl;
            if (can_decrease){
                float M = float(state->DCTCPBytesMarked) / float(state->DCTCPBytesAcked);
                EV << "SOUGOL: This is the fraction:" << M <<endl;
                EV << "SOUGOL: This is Alpha before updating: " << state->DCTCPAlpha << endl;
                state->DCTCPAlpha = float(state->DCTCPAlpha * (1 - g)) + float(g * M);
                EV << "SOUGOL: This is Alpha after updating: " << state->DCTCPAlpha << endl;
                state->DCTCPWindowEnd = state->snd_nxt;
                state->DCTCPBytesAcked = state->DCTCPBytesMarked = 0;
                EV << "SOUGOL: This is CWND before updating: " << state->snd_cwnd << endl;
                state->snd_cwnd = std::max(uint32(state->snd_cwnd * (1 - (state->DCTCPAlpha / 2))), state->snd_mss);
                EV << "SOUGOL: This is CWND after updating: " << state->snd_cwnd << endl;
                EV << "SOUGOL: This is ssthresh before updating: " << state->ssthresh << endl;
                recalculateSlowStartThreshold();
                EV << "SOUGOL: This is ssthresh after updating: " << state->ssthresh << endl;
                state->sndCwr = true;
                performSsCa = false;
                EV_INFO
                              << "ssthresh = cwnd  / 2: received ECN-Echo ACK... new ssthresh = "
                               << state->ssthresh << "\n";
                EV_INFO << "cwnd : received ECN-Echo ACK... new cwnd = "
                               << state->snd_cwnd << "\n";

                // rfc-3168 page 18:
                // The sending TCP MUST reset the retransmit timer on receiving
                // the ECN-Echo packet when the congestion window is one.
                ulong buffered = conn->get_bytes_available_in_tcp_queue();
                if (buffered > 0 && state->snd_cwnd == state->snd_mss) {
                    // If there is no outstanding data, there is no need to set timer!
                    restartRexmitTimer();
                ...
                EV << "SOUGOL: In the middle of Window for CWND processing, no ECN reaction!" << endl;
                state->sndCwr = true;
//                state->DCTCPBytesAcked = state->DCTCPBytesMarked =0;
            }
            state->gotEce = false;
...
void DCTcp::reduce_rate_based_on_FR() {
    EV << "DCTCP::reduce_rate_based_on_FR called!" << endl;
    recalculateSlowStartThreshold();
    state->snd_cwnd = state->ssthresh + 3 * state->snd_mss;
}

void DCTcp::reduce_rate_based_on_RTO() {
    EV << "DCTCP::reduce_rate_based_on_RTO called!" << endl;
    recalculateSlowStartThreshold();
    state->snd_cwnd = state->snd_mss;
}

void DCTcp::reduce_rate_based_on_ECN() {
    EV << "DCTCP::reduce_rate_based_on_ECN called!" << endl;
    recalculateSlowStartThreshold();
    state->snd_cwnd = std::max(uint32(state->snd_cwnd * (1 - (state->DCTCPAlpha / 2))), state->snd_mss);
    ulong buffered = conn->get_bytes_available_in_tcp_queue();
    if (buffered > 0 && state->snd_cwnd == state->snd_mss) {
...
void DCTcp::receivedDuplicateAck()
{
    if (!state->FRs_disabled) {
        ...
        if (state->dupacks == DUPTHRESH) {    // DUPTHRESH = 3
            ...
            // straightforward approach to "filling in" the sequence space reported
            // as missing should be a reasonable approach."
            conn->tcpMain->num_dup_acks_after_fast_retransmit++;
            sendData(false);
        }
    } else
