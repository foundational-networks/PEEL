...
{
    EV << "This is TcpReno talking!" << endl;
    ...
    if (state->dupacks >= DUPTHRESH) {    // DUPTHRESH = 3
        ...
        bool can_decrease = (now - state->eceReactionTime > state->srtt);
        ...
        if (!state->def_rate_limit_update_cwnd_once_per_rtt) {
            ...
            can_decrease_due_to_deflection = (now - state->deflection_TcpeceReactionTime > state->srtt);
        }
        ...
        if (can_decrease_due_to_deflection && state->main_packet_deflected && state->def_rate_limiting_type != DEF_NO_LIMIT) {
            if (state->def_rate_limiting_type == DEF_FR_LIMIT) {
                ...
                reduce_rate_based_on_ECE();
            } else
                throw cRuntimeError("TCP Reno: Don't recognize def_rate_limiting_type");
            state->deflection_TcpeceReactionTime = now;
            if (state->use_one_decrease_threshold)
                state->eceReactionTime = now;
...
void TcpReno::reduce_rate_based_on_FR() {
    EV << "TcpReno::reduce_rate_based_on_FR called!" << endl;
    recalculateSlowStartThreshold();
    state->snd_cwnd = state->ssthresh + 3 * state->snd_mss;
}

void TcpReno::reduce_rate_based_on_RTO() {
    EV << "TcpReno::reduce_rate_based_on_RTO called!" << endl;
    recalculateSlowStartThreshold();
    state->snd_cwnd = state->snd_mss;
}

void TcpReno::reduce_rate_based_on_ECE() {
    EV << "TcpReno::reduce_rate_based_on_ECE called!" << endl;
    state->ssthresh = state->snd_cwnd / 2;
    state->snd_cwnd = std::max(state->snd_cwnd / 2, state->snd_mss);
    ulong buffered = conn->get_bytes_available_in_tcp_queue();
    if (buffered > 0 && state->snd_cwnd == 1) {
        restartRexmitTimer();
    }
}
