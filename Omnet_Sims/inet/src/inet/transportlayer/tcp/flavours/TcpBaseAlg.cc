...
TcpBaseAlgStateVariables::TcpBaseAlgStateVariables()
{
    ...
    srtt = 0;
    rttvar = INITIAL_REXMIT_TIMEOUT / 4.0;
    ...
    if (TCP_TIMEOUT_SYN_REXMIT != INITIAL_REXMIT_TIMEOUT)
        throw cRuntimeError("SEPEHR: I don't think you want TCP_TIMEOUT_SYN_REXMIT != INITIAL_REXMIT_TIMEOUT. "
                "If you do, just comment this error out!");
    if (TCP_TIMEOUT_SYN_REXMIT_MAX != MAX_REXMIT_TIMEOUT)
            throw cRuntimeError("SEPEHR: I don't think you want TCP_TIMEOUT_SYN_REXMIT_MAX != MAX_REXMIT_TIMEOUT. "
                    "If you do, just comment this error out!");

    if (!printed_rtos) {
        printed_rtos = true;
        std::cout << "TCP-> INITIAL_RTO: " << INITIAL_REXMIT_TIMEOUT << "s, MIN_RTO: " <<
                MIN_REXMIT_TIMEOUT << "s, SYN_INITIAL_RTO: " <<
                TCP_TIMEOUT_SYN_REXMIT << "s, MAX_REXMIT_COUNT: " <<
                MAX_REXMIT_COUNT << endl;
...
TcpBaseAlg::~TcpBaseAlg()
{
    ...
    EV << "Destructor Called for: " << conn->getName()
    ...
    if (pacingTimer)
        delete cancelEvent(pacingTimer);
...
void TcpBaseAlg::initialize()
{
    ...
    pacingTimer = new cMessage("PACING");
    ...
    pacingTimer->setContextPointer(conn);
}
...
void TcpBaseAlg::established(bool active)
{
    ...
    if (state->increased_IW_enabled && state->use_custom_IW)
        throw cRuntimeError("state->increased_IW_enabled and state->use_custom_IW both are on!");
    ...
    } else if (state->use_custom_IW && state->syn_rexmit_count == 0) {
    ...
    if (active) {
        ...
        if (!sendData(false)) // FIXME TODO - This condition is never true because the buffer is empty (at this time) therefore the first ACK is never piggyback on data
            conn->sendAck(conn->rcvd_time_to_be_echod, conn->hop_count_to_be_echod, false);
    }
}
...
{
    EV << "SEPEHR: Processing Timer = " << timer << endl;
    ...
    else if (timer == pacingTimer)
        processPacingTimer(event, timer->par("pacing_type").longValue());
    else
        throw cRuntimeError(timer, "unrecognized timer");
}

void TcpBaseAlg::processPacingTimer(TcpEventCode& event, int timer_type) {
    throw cRuntimeError("TcpBaseAlg has no pacing implemented!");
}
...
void TcpBaseAlg::processRexmitTimer(TcpEventCode& event)
{
    ...
    conn->tcpMain->num_rtos++;
    EV << "SEPEHR: RTO happened. state->snd_una: " << state->snd_una << " and state->snd_nxt: " << state->snd_nxt << endl;
    conn->emit(rtoSignal, state->rexmit_timeout);
...
{
    // swift does not support delayed ack
    state->ack_now = true;
    conn->sendAck();
}
...
void TcpBaseAlg::startRexmitTimer()
{
    ...
    // schedule timer
    EV << "SEPEHR: Setting the rexmitTimer to " << simTime() + state->rexmit_timeout << endl;
    conn->scheduleTimeout(rexmitTimer, state->rexmit_timeout);
}

void TcpBaseAlg::rttMeasurementComplete(simtime_t tSent, simtime_t tAcked)
{
    EV_INFO << "SEPEHR: rttMeasurementComplete is called to update RTO." << endl;
    //
    ...
    // record statistics
    EV_DETAIL << "SEPEHR: tSent is " << tSent << " and tAcked is " << tAcked << " so measured RTT is " << tAcked - tSent << endl;
    EV_DETAIL << "Measured RTT=" << (newRTT * 1000) << "ms, updated SRTT=" << (srtt * 1000)
              << "ms, new RTO=" << (rto * 1000) << "ms\n";

    conn->emit(rttSignal, newRTT);
    conn->tcpMain->num_rtt_records++;
    conn->tcpMain->rtt_sum += newRTT.dbl();
    conn->emit(srttSignal, srtt);
    conn->emit(rttvarSignal, rttvar);
    conn->emit(rtoSignal, rto);
}

void TcpBaseAlg::rttMeasurementCompleteUsingTS(simtime_t echoedTS)
{
    EV << "SEPEHR: rttMeasurementCompleteUsingTS called." << endl;
...
bool TcpBaseAlg::sendData(bool sendCommandInvoked)
{
    ...
    EV << "SEPEHR: sendData called which is defined in TcpBaseAlg" << endl;
    ...
    if (!conn->isSendQueueEmpty()) {    // do we have any data to send?
        if (!state->use_long_running_flows && (simTime() - state->time_last_data_sent) > state->rexmit_timeout) {
...
void TcpBaseAlg::receiveSeqChanged()
{
    ...
    if (state->full_sized_segment_counter == 0 && !state->ack_now && state->last_ack_sent == state->rcv_nxt && !delayedAckTimer->isScheduled()) {    // ackSent?
        ...
        if (!state->delayed_acks_enabled) {    // delayed ACK disabled
            ...
            conn->sendAck(conn->rcvd_time_to_be_echod, conn->hop_count_to_be_echod, conn->is_packet_deflected);
        }
        else {    // delayed ACK enabled
            // swift does not support delayed ack
            // conn->is_packet_deflected is not piggybacked for delayed ack in the current version of the code
            if (state->ack_now) {
            ...
            // RFC 1122, page 96: "in a stream of full-sized segments there SHOULD be an ACK for at least every second segment."
                ...
                conn->sendAck(0,0,conn->is_packet_deflected);
            }
            else {
...
{
    EV << "SEPEHR: receivedDataAck called in TcpBaseAlg" << endl;
    if (!state->ts_enabled) {
...
//                      << floor((simTime() - state->rtseq_sendtime) * 1000 + 0.5) << "ms\n";
            EV_DETAIL << "SEPEHR: current time is " << simTime() << " and packet send time is " << state->rtseq_sendtime << " so the rtt is " << (simTime() - state->rtseq_sendtime) << endl;
...
{
    EV << "SEPEHR: dataSent called in TcpBaseAlg" << endl;
