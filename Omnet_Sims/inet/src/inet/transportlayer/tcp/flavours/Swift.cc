...
#define MAX_CWND_PACKET_NUMBER 45      // todo: borrowed from TCP. around 65536 bytes
...
//            std::cout << state->time_last_data_sent << endl;
//            std::cout << state->rexmit_timeout << endl;
//            std::cout << state->snd_cwnd << endl;
//            throw cRuntimeError("omg!!!!");
...
void Swift::receivedDataAck(uint32 firstSeqAcked)
{
    ...
    if (!state->def_rate_limit_update_cwnd_once_per_rtt) {
        ...
        if (t_last_decrease_due_to_deflection <= 0)
            can_decrease_due_to_deflection = true;
        else
            can_decrease_due_to_deflection = ((now - t_last_decrease_due_to_deflection) >= most_recent_rtt);
    }
    ...
    if (state->dupacks >= DUPTHRESH) {    // DUPTHRESH = 3
        ...
        if (can_decrease_due_to_deflection && state->main_packet_deflected && state->def_rate_limiting_type != DEF_NO_LIMIT) {
            // REGULAR reaction does not make sense for swift.
...
//                reduce_rate_based_on_regular();
//            }
            else
                throw cRuntimeError("Swift: Don't recognize def_rate_limiting_type");
            t_last_decrease_due_to_deflection = now;
        } else {
    ...
    //            throw cRuntimeError("Technically, with our currect setting this line should never execute!");
            ...
            if (state->fabric_delay < fabric_target_delay) {
                ...
                if (can_decrease) {
                    ...
                    fcwnd *= std::max(1 - state->beta *
                            ((state->fabric_delay - fabric_target_delay) / state->fabric_delay),
                            1 - state->max_mdf);
                }
            }
        }
...
void Swift::reduce_rate_based_on_FR() {
    EV_INFO << "Swift::reduce_rate_based_on_FR called!" << endl;
    ecwnd *= (1 - state->max_mdf);
    fcwnd *= (1 - state->max_mdf);
}

void Swift::reduce_rate_based_on_RTO() {
    EV_INFO << "Swift::reduce_rate_based_on_RTO called!" << endl;
    ecwnd = MIN_CWND_PACKET_NUMBER;
    fcwnd = MIN_CWND_PACKET_NUMBER;
}

//// regular in swift is just if we leave the packet be processed normally
//void Swift::reduce_rate_based_on_regular() {
//    EV_INFO << "Swift::reduce_rate_based_on_regular called!" << endl;
...
//            fcwnd += 1;
//            ecwnd += 1;
//            set_final_cwnd_values(simTime(), ecwnd-1);
