...
void Bolt::reduce_rate_based_on_FR() {
    EV_INFO << "Bolt::reduce_rate_based_on_FR called!" << endl;
    state->snd_cwnd *= (1.0 - state->max_mdf);
}

void Bolt::reduce_rate_based_on_RTO() {
    EV_INFO << "Bolt::reduce_rate_based_on_RTO called!" << endl;
    state->snd_cwnd = MIN_CWND_PACKET_NUMBER * state->snd_mss;
}

void Bolt::reduce_rate_based_on_SRC() {
    EV_INFO << "Bolt::reduce_rate_based_on_SRC called!" << endl;
    if (state->snd_cwnd > state->snd_mss)
        state->snd_cwnd -= state->snd_mss;
}
    ...
    if (state->dupacks >= DUPTHRESH) {    // DUPTHRESH = 3
        ...
        if (can_decrease_due_to_deflection && state->main_packet_deflected && state->def_rate_limiting_type != DEF_NO_LIMIT) {
            if (state->def_rate_limiting_type == DEF_FR_LIMIT) {
                ...
                reduce_rate_based_on_SRC();
            } else
                throw cRuntimeError("Bolt: Don't recognize def_rate_limiting_type");
