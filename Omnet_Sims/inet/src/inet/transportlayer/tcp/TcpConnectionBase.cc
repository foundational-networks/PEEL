...
TcpStateVariables::TcpStateVariables()
{
    ...
    deflection_TcpeceReactionTime = 0;
}
...
{
    EV << "handleMessage called in TcpConnection. msg kind is: " << msg->getKind() << endl;
...
{
    EV << "SEPEHR: ProcessTimer in TCPConnectionBase Called!" << endl;
    printConnBrief();
...
bool TcpConnection::processTCPSegment(Packet *packet, const Ptr<const TcpHeader>& tcpseg, L3Address segSrcAddr, L3Address segDestAddr)
{
    ...
    // swift
    EV << "SEPEHR: processing TCP Segment!" << endl;
    auto packet_dup = packet->dup();
    packet_dup->setFrontIteratorPosition(b(0));
    packet_dup->removeAtFront<EthernetPhyHeader>();
    auto frame = packet_dup->peekAtFront<EthernetMacHeader>();
    rcvd_time_to_be_echod = frame->getTime_packet_received_at_nic();
    hop_count_to_be_echod = frame->getHop_count();
    // last hot is not counted
    hop_count_to_be_echod += 1;
    is_packet_deflected = frame->getIs_deflected();
    EV << "SWIFT: Echoing the time packet was received: " << rcvd_time_to_be_echod << endl;
    EV << "SWIFT: Echoing the number of hops packet has traversed: " << hop_count_to_be_echod << endl;
    delete packet_dup;
    ...
    bool event_transition = performStateTransition(event);
...
{
    Enter_Method_Silent();
    EV << "SEPEHR: processing App Command!" << endl;
    EV << "SEPEHR: msg is: " << msg->str() << endl;
    EV << msg << endl;
...
void TcpConnection::notify_ip_of_connection_closed(bool close_immediately) {
    EV << "notify_ip_of_connection_closed called with close_immediately: " << close_immediately << endl;
    std::string host_name = getParentModule()->getParentModule()->getFullName();
    std::string module_path_string = host_name + ".ipv4.ip";
    Ipv4* ip_module = check_and_cast<Ipv4 *>(getModuleByPath(module_path_string.c_str()));
    ip_module->connection_closed_notif(close_immediately, source_ip, source_port, destination_ip, destination_port);
}
...
bool TcpConnection::performStateTransition(const TcpEventCode& event)
{
    ...
    bool close_instead_of_timewait = getAncestorPar("close_instead_of_timewait");
    switch (fsm.getState()) {
        case TCP_S_INIT:
            switch (event) {
                case TCP_E_OPEN_PASSIVE:
        ...
        case TCP_S_LAST_ACK:
            switch (event) {
                ...
                case TCP_E_RCV_RST:
                case TCP_E_RCV_UNEXP_SYN:
                    notify_ip_of_connection_closed(true);
                    FSM_Goto(fsm, TCP_S_CLOSED);
                    break;
        ...
        case TCP_S_FIN_WAIT_2:
            switch (event) {
                ...
                case TCP_E_RCV_FIN:
                    if (close_instead_of_timewait) {
                        notify_ip_of_connection_closed(false);
                        FSM_Goto(fsm, TCP_S_CLOSED);
                    }
                    else
                        FSM_Goto(fsm, TCP_S_TIME_WAIT);
    ...
    switch (state) {
        ...
        case TCP_S_CLOSING:
            if (state == TCP_S_CLOSE_WAIT) {
                EV << "SEPEHR: calling sendIndicationToApp with code TCP_I_PEER_CLOSED" << endl;
        ...
        case TCP_S_TIME_WAIT:
            EV << "SEPEHR: calling sendIndicationToApp with code TCP_I_CLOSED1" << endl;
            sendIndicationToApp(TCP_I_CLOSED);
            break;

        case TCP_S_CLOSED:
            if (oldState != TCP_S_TIME_WAIT && event != TCP_E_ABORT) {
               EV << "SEPEHR: calling sendIndicationToApp with code TCP_I_CLOSED2" << endl;
                sendIndicationToApp(TCP_I_CLOSED);
            }
