...
{
    EV << "SEPEHR: Printing a brief summary of connection!" << endl;
...
//    }
    ...
    if (packet_name.find("tcpseg") != std::string::npos)
        ...
        packet->addTagIfAbsent<IsBurstyTag>()->setIs_bursty(false);
...
{
    EV_INFO << "Notifying app1: " << indicationName(code) << "\n";
...
{
    EV_INFO << "Notifying app2: " << indicationName(TCP_I_AVAILABLE) << "\n";
...
{
    EV_INFO << "Notifying app3: " << indicationName(TCP_I_ESTABLISHED) << "\n";
...
void TcpConnection::sendAvailableDataToApp()
{
    if (receiveQueue->getAmountOfBufferedBytes()) {
        if (tcpMain->useDataNotification) {
            EV << "SEPEHR: Sending indication to app." << endl;
            ...
            while (auto msg = receiveQueue->extractBytesUpTo(state->rcv_nxt)) {
                EV << "SEPEHR: Sending data to app." << endl;
                tcpMain->light_goodput_num_bits_sent_to_app += msg->getBitLength();
...
void TcpConnection::initConnection(TcpOpenCommand *openCmd)
{
    ...
    // bolt
    std::string mac_module_name = tcpMain->getParentModule()->getFullPath() + ".eth[0].mac";
    EtherMacFullDuplex *mac = check_and_cast<EtherMacFullDuplex *>(getModuleByPath(mac_module_name.c_str()));
    state->server_link_rate = mac->get_link_util();
}
...
void TcpConnection::configureStateVariables()
{
    ...
    state->use_custom_IW = tcpMain->par("useCustomIW");
    state->custom_IW_mult = tcpMain->par("customIWMult");
    ...
    state->FRs_disabled = tcpMain->par("FRs_disabled");
...
//    state->ecnEnabled = tcpMain->par("ecnEnabled");
    ...
    //swift
    state->ai = tcpMain->par("additive_increase_constant");
    state->beta = tcpMain->par("multiplicative_decrease_constant");
    state->max_mdf = tcpMain->par("max_mdf");
    state->base_target_delay = tcpMain->par("base_target_delay");
    state->per_hop_scaling_factor = tcpMain->par("per_hop_scaling_factor");
    state->fs_max_cwnd = tcpMain->par("max_cwnd_target_scaling");
    state->fs_min_cwnd = tcpMain->par("min_cwnd_target_scaling");
    state->fs_range = tcpMain->par("max_scaling_range");
    state->constant_endpoint_target_delay = tcpMain->par("constant_endpoint_target_delay");
    state->endpoint_EWMA_factor = tcpMain->par("endpoint_EWMA_factor");

    // deflection rate limiting
    std::string def_rate_limit_type_str = tcpMain->par("def_rate_limit_type_str");
    if (strcmp(def_rate_limit_type_str.c_str(), "none") == 0) {
        state->def_rate_limiting_type = DEF_NO_LIMIT;
    } else if (strcmp(def_rate_limit_type_str.c_str(), "FR") == 0) {
        state->def_rate_limiting_type = DEF_FR_LIMIT;
    } else if (strcmp(def_rate_limit_type_str.c_str(), "RTO") == 0) {
        state->def_rate_limiting_type = DEF_RTO_LIMIT;
    } else if (strcmp(def_rate_limit_type_str.c_str(), "regular") == 0) {
        state->def_rate_limiting_type = DEF_REGULAR_REACTION_LIMIT;
    } else
        throw cRuntimeError("def_rate_limiting_type not specified!");
    state->use_one_decrease_threshold = tcpMain->par("use_one_decrease_threshold");
    state->def_rate_limit_update_cwnd_once_per_rtt = tcpMain->par("def_rate_limit_update_cwnd_once_per_rtt");
    state->use_long_running_flows = tcpMain->par("use_long_running_flows");
}
...
void TcpConnection::selectInitialSeqNum()
{
    ...
    state->iss_initialized_time = simTime();

    state->snd_una = state->snd_nxt = state->snd_max = state->iss;
    state->DCTCPWindowEnd = state->snd_una;
    state->deflection_DCTCPWindowEnd = state->snd_una;
...
{
    std::string dest_ip = remoteAddr.str();
    std::string conn_id = std::to_string(this->getId());
    std::hash<std::string> flow_hash;
    long hash_of_flow = flow_hash(dest_ip + conn_id);
    EV << "Syn is being sent, the info for flow hash is " <<
            dest_ip + conn_id << endl;
    EV << "Hash of flow is " << hash_of_flow << endl;
    emit(Tcp::tcpConnectionSYNSentSignal, hash_of_flow);
    emit(Tcp::tcpConnectionSYNSentIsBurstySignal, is_bursty);
...
//    }
    ...
    if (state->ecnWillingness) {
        ...
        EV << "Sougol: The ECN param, is set to true" << endl;
    } else {
        // rfc 3168 page 16:
...
void TcpConnection::sendAck(simtime_t receive_time_to_be_echoed, unsigned int hop_count_to_be_echoed, bool is_main_packet_deflected)
{
    EV << "SEPEHR: Sending Ack." << endl;
    const auto& tcpseg = makeShared<TcpHeader>();
    ...
    //swift
    EV << "SWIFT: sendAck: Setting the rcved time to be echoed to " << receive_time_to_be_echoed << endl;
    EV << "SWIFT: sendAck: Setting the hop_count to be echoed to " << hop_count_to_be_echoed << endl;
    tcpseg->setRcvd_time_to_be_echod(receive_time_to_be_echoed);
    tcpseg->setHop_count_to_be_echoed(hop_count_to_be_echoed);

    EV << "Piggy backing if packet is deflected!" << endl;
    tcpseg->setIs_main_packet_deflected(is_main_packet_deflected);

    // bolt
    EV << "Bolt: Using the ack to echo back the data packet's inc: " << state->last_data_packet_inc << endl;
    // this is only when you are sending an ack because you received an ack
    // for a data that you have not sent, so we want set inc to false

    tcpseg->setBolt_inc(state->last_data_packet_inc);
...
//    }
    ...
    // rfc-3168, pages 19-20:
        ...
        EV << "SOUGOL: THE state is: " << state << endl;
        EV << "SOUGOL: THE ect state is: " << state->ect << endl;
        if (state && state->ect) {
            EV << "SOUGOL: Do we get in here? state satisfies the condition?: "  << endl;
            ...
            if (state->ecnEchoState == true) {
                ...
                EV << "SOUGOL: ECN-Setup Exit the ECN echo state after sending one ACK! " << endl;
...
{
    EV << "TcpConnection::sendSegment called for bytes: " << bytes << endl;
    ...
    if (state->sack_enabled && state->afterRto) {
        ...
        if (forward > 0) {
            EV << "Sepehr this is a reminder to you about sack's problem with bytes = 0" << endl;
//            throw cRuntimeError("Sepehr this is a reminder to you about sack's problem with bytes = 0");
    ...
    EV << "Bytes is " << bytes << " and buffered is " << buffered << endl;
    if (bytes > buffered) {
        // last segment?
        bytes = buffered;
    }
    ...
    EV << "SEPEHR: payload created. packet is " << packet->str() << endl;
//    packet->addTag<SocketTagBase>()->setSocketId(12345);
    const auto& tcpseg = makeShared<TcpHeader>();
    tcpseg->setSequenceNo(state->snd_nxt);
    ...
    // bolt
    if (tcpAlgorithm->is_bolt()) {

        if (!state->use_custom_IW && state->custom_IW_mult != 1) {
            throw cRuntimeError("For bolt, if we are not using custom IW, you should set the mult to 1 as we need"
                    " it for marking the first window of packets");
        }

        EV << "Setting Bolt's inc to true for " << tcpseg->str() << endl;
        tcpseg->setBolt_inc(true);

        // bolt: Setting first
        EV << "state->snd_nxt: " << state->snd_nxt << endl;
        EV << "state->iss: " << state->iss << endl;
        if (state->snd_nxt - state->iss < state->custom_IW_mult * state->snd_mss) {
            EV << "Setting Bolt's first flag to true" << endl;
            tcpseg->setBolt_first(true);
        }

        if (bytes >= buffered) {
            // bolt: last segment
            EV << "Setting Bolt's last flag to true" << endl;
            tcpseg->setBolt_last(true);
        }
    }
...
ulong TcpConnection::get_bytes_available_in_tcp_queue()
{
    if (!sendQueue)
        throw cRuntimeError("Trying to read the bytes available in sendQeueu while sendQueue is null!");
    return sendQueue->getBytesAvailable(state->snd_nxt);
}

bool TcpConnection::sendData(bool fullSegmentsOnly, uint32 congestionWindow)
{
    EV << "SEPEHR: Send Data is called in TCPConnectionUnit" << endl;
    ...
    ulong buffered = sendQueue->getBytesAvailable(state->snd_nxt);
    EV << "SEPEHR: number of bytes available in tcp buffer: " << buffered << endl;
    ...
    EV << "SEPEHR: snd_wnd = " << state->snd_wnd << " and congestionWindow = " << congestionWindow << " so maxWindow = " << maxWindow << endl;

    // effectiveWindow: number of bytes we're allowed to send now
    long effectiveWin = maxWindow - (state->snd_nxt - state->snd_una);
    EV << "SEPEHR: snd_nxt = " << state->snd_nxt << " and snd_una = " << state->snd_una << " so effectiveWin = " << effectiveWin << endl;
    ...
    if (bytesToSend <= state->snd_mss) {
        EV << "Calling sendSegment in if statement: bytesToSend <= state->snd_mss" << endl;
...
//        while (bytesToSend >= effectiveMaxBytesSend && sendQueue->getBytesAvailable(state->snd_nxt) > 0) {
            EV << "Calling sendSegment in while statement: bytesToSend >= effectiveMaxBytesSend" << endl;
            EV << "effectiveMaxBytesSend: " << effectiveMaxBytesSend << endl;
            EV << "bytesToSend: " << bytesToSend << endl;
            sendSegment(state->snd_mss);
            ASSERT(bytesToSend >= state->sentBytes);
            EV << "sentBytes: " << state->sentBytes << endl;
            if (state->sentBytes <= 0)
                break;
            bytesToSend -= state->sentBytes;
        }
    }
    ...
    if (bytesToSend == buffered && buffered != 0) { // last segment?
        EV << "Calling sendSegment in if statement: bytesToSend == buffered && buffered != 0" << endl;
        sendSegment(bytesToSend);
    }
...
{
    tcpMain->num_segment_retransmissions++;
//    //ECN
//    if (state->ect && state->sndCwr) {
//        tcpseg->setCwrBit(true);
//        EV_INFO << "set CWR bit\n";
...
bool TcpConnection::processTSOption(const Ptr<const TcpHeader>& tcpseg, const TcpOptionTimestamp& option)
{
    ...
    if (!state->ts_enabled) {
        ...
        EV << "SEPEHR: state->ts_support: " << state->ts_support << ", state->snd_initial_ts: " <<  state->snd_initial_ts << ", state->rcv_initial_ts: " << state->rcv_initial_ts << " so state->ts_enabled: " << state->ts_enabled << endl;
    ...
    bool connection_starting =  state->last_ack_sent == 0 && fsm.getState() == TCP_S_SYN_SENT;

    if (state->ts_enabled) {
        if (option.getSenderTimestamp() < state->ts_recent) {
        ...
        else if (seqLE(tcpseg->getSequenceNo(), state->last_ack_sent) || connection_starting) {    // Note: test is modified according to the latest proposal of the tcplw@cray.com list (Braden 1993/04/26)
...
TcpHeader TcpConnection::writeHeaderOptions(const Ptr<TcpHeader>& tcpseg)
{
                                    ...
                                    && state->syn_rexmit_count > 0)))
    {
                                                            ...
                                                            || (fsm.getState() == TCP_S_SYN_SENT && state->syn_rexmit_count > 0))))
        {
            ...
            option->setSenderTimestamp(simTime());
            ...
            // must be zero."
            EV << "SEPEHR: state->ts_recent is " << state->ts_recent << ", tcpseg->getAckBit(): " << tcpseg->getAckBit() << " Setting echoedtimestamp1 to " << (tcpseg->getAckBit() ? state->ts_recent : 0) << endl;
//            throw cRuntimeError("Gotchaa1!!!!");
             ...
             || fsm.getState() == TCP_S_FIN_WAIT_2 || fsm.getState() == TCP_S_CLOSE_WAIT)    // connetion is not in INIT or LISTEN state
    {
        ...
        if (state->ts_enabled) {    // Is TS enabled?
            ...
            // of a Timestamps option.  When TSecr is not valid, its value
            // must be zero."
            EV << "SEPEHR: state->ts_recent is " << state->ts_recent << ", tcpseg->getAckBit(): " << tcpseg->getAckBit() << " Setting echoedtimestamp2 to " << (tcpseg->getAckBit() ? state->ts_recent : 0) << endl;
//            throw cRuntimeError("Gotchaa2!!!!");
            option->setEchoedTimestamp(tcpseg->getAckBit() ? state->ts_recent : 0);
    ...
    EV << "Header options written" << endl;
    return *tcpseg;
}

simtime_t TcpConnection::getTSval(const Ptr<const TcpHeader>& tcpseg) const
{
    for (uint i = 0; i < tcpseg->getHeaderOptionArraySize(); i++) {
        const TcpOption *option = tcpseg->getHeaderOption(i);
...
simtime_t TcpConnection::getTSecr(const Ptr<const TcpHeader>& tcpseg) const
{
    for (uint i = 0; i < tcpseg->getHeaderOptionArraySize(); i++) {
        const TcpOption *option = tcpseg->getHeaderOption(i);

        if (option->getKind() == TCPOPTION_TIMESTAMP) {
            simtime_t time_stamp = check_and_cast<const TcpOptionTimestamp *>(option)->getEchoedTimestamp();
            EV <<"SEPEHR: tcp option kind is TCPOPTION_TIMESTAMP and time stamp is: " << time_stamp << endl;
            return time_stamp;
