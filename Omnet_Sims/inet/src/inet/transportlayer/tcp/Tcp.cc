...
bool is_packet_ndp(Packet *packet) {
    ...
    delete packet_dup;
    return eth_header->getIs_v2_dropped_packet_header();
}

void Tcp::handleLowerPacket(Packet *packet)
{
    EV << "SEPEHR: Handling Lower Packet: " << packet->str() << endl;
    if (is_packet_ndp(packet)) {
        EV << "Packet is a drop notification packet! No need for it in transport. Discarding it." << endl;
        delete packet;
        return;
    }
    ...
    if (protocol == &Protocol::tcp) {
        ...
        // bypassing for bolt SRC packets
        if (packet_name.find("SRC") == std::string::npos && !checkCrc(packet)){
//        if (!checkCrc(packet)){
            throw cRuntimeError("Tcp: here for the sake of bolt");
...
void Tcp::removeConnection(TcpConnection *conn)
{
    ...
    EV << "SEPEHR: conn->socketId is: " << conn->socketId << endl;

//    conn->num_dup_acks
    if (!conn->connection_information_stored)
        throw cRuntimeError("How isn't the connection information stored and the connection is being closed");
