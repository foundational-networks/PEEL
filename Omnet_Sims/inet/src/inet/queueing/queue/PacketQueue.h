...
#define QUEUE_QUANTILE_SEL_REACTION 0
#define QUEUE_NONBURSTY_ONLY_SEL_REACTION 1
...
class INET_API PacketQueue : public PacketQueueBase, public IPacketBuffer::ICallback
{
  ...
  public:
    ...
    virtual bool is_over_v2_threshold_full(b packet_length, Packet* packet, long on_the_way_packet_num = 0, b on_the_way_packet_length = b(0)) {
        throw cRuntimeError("is_over_v2_threshold_full called in PacketQueue");
    }
    ...
    virtual bool get_packet_deflection_tag(Packet *packet);
    virtual void set_BW_bps(double bandwidth) {throw cRuntimeError("set_BW_bps called!");}
    virtual void calculate_suply_token(int packet_size_bits) {throw cRuntimeError("calculate_suply_token called!");}

    virtual void insert_front_of_queue(Packet* packet) override;
    cPacket* get_index(unsigned int idx) override;
};
