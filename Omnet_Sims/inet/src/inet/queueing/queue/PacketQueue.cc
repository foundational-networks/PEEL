...
PacketQueue::~PacketQueue() {
    recordScalar("lightFinalPFCStatus", pfc_status);
    recordScalar("lightFinalPFCCounter", pfc_bytes_counter.get());
...
void PacketQueue::pushPacket(Packet *packet, cGate *gate)
{
    ...
    if (collector != nullptr && getNumPackets() != 0){
        ...
        throw cRuntimeError("PacketQueue::Collector is not nullptr!.");
        collector->handleCanPopPacket(outputGate);
    }
}

cPacket* PacketQueue::get_index(unsigned int idx) {
    return queue.get(idx);
}

void PacketQueue::insert_front_of_queue(Packet* packet) {
    if (queue.getLength() > 0)
        queue.insertBefore(queue.get(0), packet);
    else
        queue.insert(packet);
}
...
Packet *PacketQueue::popPacket(cGate *gate)
{
    ...
    EV_INFO << "Popping packet " << packet->getName() << " from the queue. Queue length: " << getNumPackets() << endl;
