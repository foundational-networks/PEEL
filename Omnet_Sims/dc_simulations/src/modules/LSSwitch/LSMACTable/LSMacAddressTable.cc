...
std::list<MacAddress> LSMacAddressTable::getAllAddresses()
{
    std::list<MacAddress> allAddresses;
    for (const auto & vlanPair : vlanAddressTable) {
        AddressTable *table = vlanPair.second;
        if (table == nullptr)
            continue;

        for (const auto & macPair : *table) {
            allAddresses.push_back(macPair.first);
        }
    }
    return allAddresses;
}
...
// SEPEHR: add removers for failure
bool LSMacAddressTable::removeInterfaceForAddress(const MacAddress& address, int interfaceId, unsigned int vid)
{
    Enter_Method("LSMacAddressTable::removeInterfaceForAddress()");
    AddressTable *table = getTableForVid(vid);
    if (table == nullptr)
        return false;
    ...
    // Walk the two lists in lock-step
    auto itIf  = entry.interfaceList.begin();
    auto itTs  = entry.insertionTimeList.begin();
    for ( ; itIf != entry.interfaceList.end() && itTs != entry.insertionTimeList.end(); /* advanced inside */ ) {
        if (*itIf == interfaceId) {
            EV << "Removing entry (vid=" << vid << "): " << address
               << " --> interfaceId " << *itIf << " at time " << *itTs << "\n";
            itIf = entry.interfaceList.erase(itIf);
            itTs = entry.insertionTimeList.erase(itTs);

            // If this MAC has no more ports, drop the whole map entry.
            if (entry.interfaceList.empty()) {
                table->erase(it);
            }
            return true; // remove only one occurrence; switch to "continue" to purge duplicates
        }
        else {
            ++itIf;
            ++itTs;
...
bool LSMacAddressTable::removeInterfaceForAddressAllVlans(const MacAddress& address, int interfaceId)
{
    Enter_Method("LSMacAddressTable::removeInterfaceForAddressAllVlans()");
    bool removed = false;
    ...
    for (auto &elem : vlanAddressTable) {
        ...
        for ( ; itIf != entry.interfaceList.end() && itTs != entry.insertionTimeList.end(); /* advanced inside */ ) {
            if (*itIf == interfaceId) {
                EV << "Removing entry (vid=" << elem.first << "): " << address
                   << " --> interfaceId " << *itIf << " at time " << *itTs << "\n";
                itIf = entry.interfaceList.erase(itIf);
                itTs = entry.insertionTimeList.erase(itTs);
                removedHere = true;
                // remove only the first match on this VLAN; comment out the next line to remove all duplicates
                break;
            }
            else {
                ++itIf;
...
// SEPEHR add interface upon recovery
bool LSMacAddressTable::hasInterfaceForAddress(const MacAddress& address,
                                               int interfaceId,
                                               unsigned int vid)
{
    AddressTable *table = getTableForVid(vid);
    ...
    const AddressEntry& entry = it->second;

    auto itIf = entry.interfaceList.begin();
    auto itTs = entry.insertionTimeList.begin();
    for ( ; itIf != entry.interfaceList.end() && itTs != entry.insertionTimeList.end(); ++itIf, ++itTs) {
        if (*itIf == interfaceId)
            return true;
    }
    return false;
...
bool LSMacAddressTable::ensureInterfaceForAddress(const MacAddress& address,
                                                  int interfaceId,
                                                  unsigned int vid,
                                                  bool updateTimestampIfExists)
{
    Enter_Method("LSMacAddressTable::ensureInterfaceForAddress()");
    if (address.isMulticast())
        return false; // keep parity with updateTableWithAddress()

    // Optionally prune old stuff before touching the table
    removeAgedEntriesIfNeeded();

    AddressTable *table = getTableForVid(vid);

    // Create VLAN table if needed
    if (table == nullptr) {
    ...
    // If MAC not present on this VLAN, create a fresh entry
    if (it == table->end()) {
        std::list<int> ifList;
        std::list<simtime_t> tsList;
        ifList.push_back(interfaceId);
        tsList.push_back(simTime());

        EV << "Adding new MAC entry (vid=" << vid << "): " << address
           << " --> interfaceId " << interfaceId << "\n";

        (*table)[address] = AddressEntry(vid, ifList, tsList);
        return true; // inserted new pair
    }

    // MAC exists: scan for the interface in lock-step
    AddressEntry& entry = it->second;
    ...
    for ( ; itIf != entry.interfaceList.end() && itTs != entry.insertionTimeList.end(); ++itIf, ++itTs) {
        if (*itIf == interfaceId) {
            // already present
            if (updateTimestampIfExists) {
                *itTs = simTime();
                EV << "Refreshed timestamp for (vid=" << vid << "): " << address
                   << " --> interfaceId " << interfaceId << "\n";
            }
            return false; // not inserted (already existed)
        }
    }
    ...
    EV << "Appended interface to existing MAC (vid=" << vid << "): " << address
       << " --> interfaceId " << interfaceId << "\n";
    return true; // inserted new pair
}
