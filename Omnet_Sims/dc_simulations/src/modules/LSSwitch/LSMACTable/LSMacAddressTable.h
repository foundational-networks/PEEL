...
#ifndef LSMACADDRESSTABLE_H
#define LSMACADDRESSTABLE_H

#include "inet/common/lifecycle/ModuleOperations.h"
#include "inet/common/lifecycle/OperationalBase.h"
#include "inet/linklayer/common/MacAddress.h"
#include "./LSIMacAddressTable.h"
 ...
 */
class LSMacAddressTable : public OperationalBase, public LSIMacAddressTable
{
  protected:
    struct AddressEntry
    {
        std::list<int> interfaceList;
        std::list<simtime_t> insertionTimeList;
...
//        simtime_t insertionTime;    // Arrival time of Lookup Address Table entry
        AddressEntry() {}
        AddressEntry(unsigned int vid, std::list<int> interfaceList, std::list<simtime_t> insertionTimeList) :
            vid(vid), interfaceList(interfaceList), insertionTimeList(insertionTimeList) {}
    };
  ...
  protected:
     ...
     */
    virtual AddressTable *getTableForVid(unsigned int vid);

  public:

    LSMacAddressTable();
    ~LSMacAddressTable();
    virtual std::list<MacAddress> getAllAddresses() override;

  public:
    // SEPEHR: add removers for failure
    /**
     * @brief Remove exactly one (address, interfaceId) entry on a given VLAN.
     * @return true if removed, false if not found.
     */
    virtual bool removeInterfaceForAddress(const MacAddress& address, int interfaceId, unsigned int vid = 0) override;

    /**
     * @brief Remove (address, interfaceId) across all VLANs.
     * @return true if at least one entry was removed.
     */
    virtual bool removeInterfaceForAddressAllVlans(const MacAddress& address, int interfaceId) override;
     ...
     */
    virtual std::list<int> getInterfaceIdForAddress(const MacAddress& address, unsigned int vid = 0) override;

    // SEPEHR add interface upon recovery
    /**
     * @brief Ensure (address, interfaceId) exists on VLAN vid.
     * If already present, optionally update its timestamp.
     * @return true if a new entry was inserted; false if it already existed.
     */
    virtual bool ensureInterfaceForAddress(const MacAddress& address,
                                   int interfaceId,
                                   unsigned int vid = 0,
                                   bool updateTimestampIfExists = true) override;

    /**
     * @brief Check if (address, interfaceId) exists on VLAN vid.
     */
    virtual bool hasInterfaceForAddress(const MacAddress& address,
                                int interfaceId,
                                unsigned int vid = 0) override;

    virtual std::list<simtime_t> getInsertionTimeForAddress(const MacAddress& address, unsigned int vid = 0) override;
