...
#ifndef LSIMACADDRESSTABLE_H
#define LSIMACADDRESSTABLE_H
...
class LSIMacAddressTable
{
  public:
     ...
     */
    virtual bool removeInterfaceForAddress(const MacAddress& address, int interfaceId, unsigned int vid = 0) = 0;

    /**
     * @brief Remove (address, interfaceId) across all VLANs.
     * @return true if at least one entry was removed.
     */
    virtual bool removeInterfaceForAddressAllVlans(const MacAddress& address, int interfaceId) = 0;
    /**
     ...
     */
    virtual std::list<int> getInterfaceIdForAddress(const MacAddress& address, unsigned int vid = 0) = 0;

    virtual std::list<simtime_t> getInsertionTimeForAddress(const MacAddress& address, unsigned int vid = 0) = 0;
    ...
    virtual bool ensureInterfaceForAddress(const MacAddress& address,
                                   int interfaceId,
                                   unsigned int vid = 0,
                                   bool updateTimestampIfExists = true) = 0;
    ...
    virtual std::list<MacAddress> getAllAddresses() = 0;
};
