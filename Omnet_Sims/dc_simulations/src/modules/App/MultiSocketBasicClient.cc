...
void MultiSocketBasicClient::connect_for_bursty_request() {
    ...
    for (int i = 0; i < num_requests_per_burst; i++){
        ...
        connect(server_idx, 80, is_bursty, query_id);
...
void MultiSocketBasicClient::socketEstablished(TcpSocket *socket)
{
    ...
    if(use_jitter) {
        ...
        emit(notJitteredRequestSentSignal, now);
    }
    EV << "SEPEHR: jitter is: " << jitter << endl;
    EV << "SEPEHR: Scheduling a request for " << now + jitter << endl;
//    socket_ids_for_requests.push_back(socket->getSocketId())

    rescheduleOrDeleteTimer(now + jitter, MSGKIND_SEND, socket->getSocketId());
}
