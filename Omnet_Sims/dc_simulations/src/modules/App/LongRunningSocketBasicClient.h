...
#ifndef __INET_LONGRUNNINGSOCKETBASICCLIENT_H
#define __INET_LONGRUNNINGSOCKETBASICCLIENT_H
...
#include "base/LongRunningSocketTcpAppBase.h"
...
//        CustomMsgInfo() {}
        ...
        CustomMsgInfo(unsigned long flow_id, int flow_size, unsigned long query_id=0) {
 ...
 */
class INET_API LongRunningSocketBasicClient : public LongRunningSocketTcpAppBase
{
  protected:
//    cMessage *timeoutMsg = nullptr;
    ...
    // This maps a destination to the messages waiting for it to be connected
    std::map<int, std::list<CustomMsgInfo*>> destidx_msg_mapper;
    // not in dict: we should connect, in dict but false, just add message and wait
    // in dict and true, send message
    std::map<int, bool> is_destination_connected;

    virtual void sendRequest(int server_idx, CustomMsgInfo* msg_info);
    ...
    virtual void send_for_bursty_request();
    virtual void send_for_background_request();

  public:
    LongRunningSocketBasicClient() {}
    virtual ~LongRunningSocketBasicClient();
