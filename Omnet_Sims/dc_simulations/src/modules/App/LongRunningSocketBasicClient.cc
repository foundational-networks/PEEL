...
#include "./LongRunningSocketBasicClient.h"
...
Define_Module(LongRunningSocketBasicClient);

simsignal_t LongRunningSocketBasicClient::flowEndedSignal = registerSignal("flowEnded");
simsignal_t LongRunningSocketBasicClient::flowEndedQueryIDSignal = registerSignal("flowEndedQueryID");
simsignal_t LongRunningSocketBasicClient::flowStartedSignal = registerSignal("flowStarted");
simsignal_t LongRunningSocketBasicClient::actualFlowStartedTimeSignal = registerSignal("actualFlowStartedTime");
simsignal_t LongRunningSocketBasicClient::requestSentSignal = registerSignal("requestSent");
simsignal_t LongRunningSocketBasicClient::notJitteredRequestSentSignal = registerSignal("notJitteredRequestSent");
simsignal_t LongRunningSocketBasicClient::replyLengthsSignal = registerSignal("replyLengths");
simsignal_t LongRunningSocketBasicClient::chunksReceivedLengthSignal = registerSignal("chunksReceivedLength");
simsignal_t LongRunningSocketBasicClient::chunksReceivedTotalLengthSignal = registerSignal("chunksReceivedTotalLength");



LongRunningSocketBasicClient::~LongRunningSocketBasicClient()
{
}

void LongRunningSocketBasicClient::initialize(int stage)
{
    LongRunningSocketTcpAppBase::initialize(stage);
...
void LongRunningSocketBasicClient::handleStartOperation(LifecycleOperation *operation)
{
...
//            throw cRuntimeError("You created an app with < 1 number of requests. Are you sure!?");
    ...
    if (((stopTime < SIMTIME_ZERO) || (start < stopTime) || (start == stopTime && startTime == stopTime))) {
        rescheduleOrDeleteTimer(get_inter_arrival_time(), MSGKIND_SEND);
    }
}

void LongRunningSocketBasicClient::send_for_bursty_request() {

    EV << "LongRunningSocketBasicClient::send_for_bursty_request" << endl;
    ...
    for (int i = 0; i < num_requests_per_burst; i++){
        ...
        auto dest_server_idx_found = is_destination_connected.find(server_idx);
        if (dest_server_idx_found == is_destination_connected.end()) {
            // should connect to server
            EV << "Should connect to destination!" << endl;
            is_destination_connected.insert(std::pair<int, bool>(server_idx, false));
            auto destidx_msg_mapper_found = destidx_msg_mapper.find(server_idx);
            if (destidx_msg_mapper_found != destidx_msg_mapper.end())
                throw cRuntimeError("bursty: The connection is just being created, how do we have info for it in destidx_msg_mapper");
            std::list<CustomMsgInfo*> waiting_messages;
            waiting_messages.push_back(new CustomMsgInfo(get_flow_id(), get_flow_size(), query_id));
            destidx_msg_mapper.insert(std::pair<int, std::list<CustomMsgInfo*>>(server_idx, waiting_messages));
            connect(server_idx, 80);
        } else if (!dest_server_idx_found->second) {
            // waiting for server to be connected
            EV << "Waiting for connection to destination to be complete!" << endl;
            auto destidx_msg_mapper_found = destidx_msg_mapper.find(server_idx);
            if (destidx_msg_mapper_found == destidx_msg_mapper.end())
                throw cRuntimeError("bursty: If the connected is being created, how can't we find server in destidx_msg_mapper?");
            destidx_msg_mapper_found->second.push_back(new CustomMsgInfo(get_flow_id(), get_flow_size(), query_id));
        } else {
            EV << "Already connected to destination. Sending request!" << endl;
            // flush the waiting messages and send this message as well
            auto waiting_messages_found = destidx_msg_mapper.find(server_idx);
            if (waiting_messages_found != destidx_msg_mapper.end() && waiting_messages_found->second.size() > 0)
                throw cRuntimeError("bursty: waiting_messages_found->second.size() > 0");
            sendRequest(server_idx, new CustomMsgInfo(get_flow_id(), get_flow_size(), query_id));
        }
    }
...
void LongRunningSocketBasicClient::send_for_background_request() {
    EV << "LongRunningSocketBasicClient::send_for_background_request" << endl;
    int server_idx = get_server_idx();
    auto dest_server_idx_found = is_destination_connected.find(server_idx);
    if (dest_server_idx_found == is_destination_connected.end()) {
        // should connect to server
        ...
        auto destidx_msg_mapper_found = destidx_msg_mapper.find(server_idx);
        if (destidx_msg_mapper_found != destidx_msg_mapper.end())
            throw cRuntimeError("bg: The connection is just being created, how do we have info for it in destidx_msg_mapper");
        std::list<CustomMsgInfo*> waiting_messages;
        waiting_messages.push_back(new CustomMsgInfo(get_flow_id(), get_flow_size()));
        destidx_msg_mapper.insert(std::pair<int, std::list<CustomMsgInfo*>>(server_idx, waiting_messages));
        connect(server_idx, 80);
    } else if (!dest_server_idx_found->second) {
        // waiting for server to be connected
        ...
        auto destidx_msg_mapper_found = destidx_msg_mapper.find(server_idx);
        if (destidx_msg_mapper_found == destidx_msg_mapper.end())
            throw cRuntimeError("bg: If the connected is being created, how can't we find server in destidx_msg_mapper?");
        destidx_msg_mapper_found->second.push_back(new CustomMsgInfo(get_flow_id(), get_flow_size()));
    } else {
        // flush the waiting messages and send this message as well
        EV << "Already connected to destination. Sending request!" << endl;
        auto waiting_messages_found = destidx_msg_mapper.find(server_idx);
        if (waiting_messages_found != destidx_msg_mapper.end() && waiting_messages_found->second.size() > 0)
            throw cRuntimeError("background: waiting_messages_found->second.size() > 0");
        sendRequest(server_idx, new CustomMsgInfo(get_flow_id(), get_flow_size()));
    }
    rescheduleOrDeleteTimer(get_inter_arrival_time(), MSGKIND_SEND);
}

void LongRunningSocketBasicClient::handleTimer(cMessage *msg)
{
    switch (msg->getKind()) {
        case MSGKIND_SEND:
            EV << "MSGKIND_SEND received" << endl;
            if (is_bursty)
                send_for_bursty_request();
            else
                send_for_background_request();
            delete msg;
            break;
...
void LongRunningSocketBasicClient::socketEstablished(TcpSocket *socket)
{
    LongRunningSocketTcpAppBase::socketEstablished(socket);
    EV << "SEPEHR: socket established!" << endl;
    simtime_t now = simTime();


    if(use_jitter) {
        throw cRuntimeError("use_jitter is not supported in long running connection!");
    }

    auto destination_server_idx_itr = socket_to_destination_mapper.find(socket);
    if (destination_server_idx_itr == socket_to_destination_mapper.end())
        throw cRuntimeError("How is the destination server not in socket_to_destination_mapper");

    auto dest_server_idx_found = is_destination_connected.find(destination_server_idx_itr->second);
    if (dest_server_idx_found == is_destination_connected.end())
        throw cRuntimeError("The connection is established, how can't we find it in is_destination_connected");
    dest_server_idx_found->second = true;

    // flush the waiting messages and send this message as well
    auto waiting_messages_found = destidx_msg_mapper.find(destination_server_idx_itr->second);
    if (waiting_messages_found != destidx_msg_mapper.end()) {
        EV << "Flushing the waiting requests. request#: " << waiting_messages_found->second.size() << endl;
        while (waiting_messages_found->second.size() > 0) {
            auto msg_info = waiting_messages_found->second.front();
            waiting_messages_found->second.pop_front();
            sendRequest(destination_server_idx_itr->second, msg_info);
        }
    }

}

void LongRunningSocketBasicClient::handleStopOperation(LifecycleOperation *operation)
{
    EV << "LongRunningSocketBasicClient::handleStopOperation called." << endl;
...
void LongRunningSocketBasicClient::handleCrashOperation(LifecycleOperation *operation)
{
    EV << "LongRunningSocketBasicClient::handleCrashOperation." << endl;
...
void LongRunningSocketBasicClient::sendRequest(int server_idx, CustomMsgInfo* msg_info)
{

    EV << "SEPEHR: SendRequest Called." << endl;
    EV << "dest idx, flow id, response size, query id" << endl;
    EV << server_idx << ", " << msg_info->flow_id << ", " << msg_info->flow_size << ", " << msg_info->query_id << endl;
    auto socket_found = destination_to_socket_mapper.find(server_idx);
    ...
    long reply_length = msg_info->flow_size;
    if (requestLength < 1)
...
static int callback_inter_arrival(void *data, int argc, char **argv, char **azColName){
   int i;
   LongRunningSocketBasicClient *app_object = (LongRunningSocketBasicClient*) data;
...
void LongRunningSocketBasicClient::read_value_from_db(std::string inter_arrival_db_path, std::string flow_size_db_path,
...
int LongRunningSocketBasicClient::get_server_idx() {
    int server_idx;
    if (is_bursty) {
...
double LongRunningSocketBasicClient::get_inter_arrival_time() {
...
int LongRunningSocketBasicClient::get_flow_size() {
    if (is_bursty) {
        return replyLength;
    }
    else {
...
unsigned long LongRunningSocketBasicClient::get_flow_id() {
...
unsigned long LongRunningSocketBasicClient::get_query_id() {
...
void LongRunningSocketBasicClient::rescheduleOrDeleteTimer(simtime_t d, short int msgKind, long socket_id)
{
...
void LongRunningSocketBasicClient::socketDataArrived(TcpSocket *socket, Packet *msg, bool urgent)
{
    ...
    while (true) {
        ...
        if (chunk_offset == b(0)) {
            ...
            simtime_t creation_time = payload->getTag<CreationTimeTag>()->getCreationTime();
            EV << "creation time is " << creation_time << endl;
            // start time should actually be the time that the first packet (whether re-ordered or not is received)
            if (payload->getRequested_time() > creation_time) {
                std::cout << "getRequested_time(): " << payload->getRequested_time() << endl;
                std::cout << "creation_time: " << creation_time << endl;
...
//            simtime_t actual_start_time = std::min(region_creation_time, simTime());
            ...
            emit(actualFlowStartedTimeSignal, creation_time);
            delete temp;
        }
    ...
    delete msg_dup;
    LongRunningSocketTcpAppBase::socketDataArrived(socket, msg, urgent);
    EV_INFO << "reply arrived\n";
    if (should_close) {
        EV << "SEPEHR: Last packet arrived. Since it's long running we keep the socket open." << endl;
//        close(socket->getSocketId());
    }
...
void LongRunningSocketBasicClient::close(int socket_id)
{
    EV << "SEPEHR: closing the socket in server[" << parent_index << "].app["
                << app_index << "] with socket ID " << socket_id
                << ". Time = " << simTime() << endl;
    LongRunningSocketTcpAppBase::close(socket_id);
}

void LongRunningSocketBasicClient::finish()
{
    ...
    is_destination_connected.clear();

    for(std::map<int, std::list<CustomMsgInfo*>>::iterator destidx_msg_mapper_it = destidx_msg_mapper.begin();
            destidx_msg_mapper_it != destidx_msg_mapper.end(); destidx_msg_mapper_it++) {
        while (destidx_msg_mapper_it->second.size() > 0) {
            delete destidx_msg_mapper_it->second.front();
            destidx_msg_mapper_it->second.pop_front();
        }
    }
    destidx_msg_mapper.clear();
    LongRunningSocketTcpAppBase::finish();
}

void LongRunningSocketBasicClient::socketClosed(TcpSocket *socket)
{
    EV << "SEPEHR: Socket closed in server[" << parent_index << "].app["
            << app_index << "] with socket ID " << socket->getSocketId()
            << ". Time = " << simTime() << endl;
    LongRunningSocketTcpAppBase::socketClosed(socket);
}

void LongRunningSocketBasicClient::socketFailure(TcpSocket *socket, int code)
{
    LongRunningSocketTcpAppBase::socketFailure(socket, code);
}
