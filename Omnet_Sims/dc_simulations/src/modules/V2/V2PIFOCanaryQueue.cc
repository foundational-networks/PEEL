...
void V2PIFOCanaryQueue::initialize(int stage) {
    ...
    just_prioritize_bursty_flows = par("just_prioritize_bursty_flows");// selective reaction
    ...
    if (apply_selective_net_reaction) {
        if (selective_net_reaction_type_string.find("quantile") != std::string::npos) {
            throw cRuntimeError("quantile sel reaction not implemented in canaryQeueu!");
        }
...
//            if (mark_packet_allowed && eth_header->getQueue_occupancy() >= dctcp_thresh) {
            if (mark_packet_allowed && getNumPackets() >= dctcp_thresh) {
