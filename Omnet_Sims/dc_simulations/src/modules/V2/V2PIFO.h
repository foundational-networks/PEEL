...
class INET_API V2PIFO : public PacketQueue
{
  ...
  protected:
    ...
    static simsignal_t poppedBytesSignal;   // aggregate popped bytes
...
//    std::string incremental_deployment_file_name;
    ...
    // pfc
    bool use_pfc = false;
    double pfc_stop_perc = -1;
    int pfc_resume_gap = -1;

    // udp
    bool use_udp = false;

    cModule *switch_module;
    cModule *relay_module;

    simtime_t last_packet_popped_time = 0;
    ...
    int extract_collective_alg(Packet *packet);
  ...
  public:
    ...
    b non_pfc_queue_occupancy = b(0);

    // light bps
    unsigned long long popped_data_pkt_num = 0;
    unsigned long popped_bytes_num = 0;
    simtime_t record_start_time = 0;


  public:
    virtual ~V2PIFO();
    virtual bool is_allowed_to_send_by_PFC(Packet* current_tx_packet) override;
    virtual B calc_pfc_thresh(b free_space);
    ...
    void mark_packet_dctcp(Packet* packet, int queue_occupancy);
    void mark_packet_red(Packet* packet, int queue_occupancy);
