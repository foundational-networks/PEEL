...
void V2PIFOPrioQueue::initialize(int stage) {
    ...
    else if (priority_mapping_scheme_str.compare("FIFO") == 0)
        priority_mapping_scheme = PRIO_FIFO;
    else
        throw cRuntimeError("No priority mapping scheme identified!");

    if (priority_mapping_scheme == PRIO_FIFO) {
        default_prio_queue_idx = par("default_prio_queue_idx");
        if (default_prio_queue_idx < 0 || default_prio_queue_idx >= num_queues)
            throw cRuntimeError("default_prio_queue_idx < 0 || default_prio_queue_idx >= num_queues");
    }
...
unsigned long V2PIFOPrioQueue::extract_priority(Packet *packet, bool is_packet_being_dropped) {
    ...
    if (priority_mapping_scheme == PRIO_FIFO) {
        int temp_prio_queue_idx = etherheader->getPrio_queue_idx();
        if (temp_prio_queue_idx == -1)
            return default_prio_queue_idx;
        else
            return temp_prio_queue_idx;
    }
        return etherheader->getPrio_queue_idx();
...
int V2PIFOPrioQueue::map_priority_to_queue_idx(unsigned long priority) {
    ...
    if (priority_mapping_scheme == SPPIFO)
        ...
        // we only have one queue and its idx is 0
        return 0;
    } else if (priority_mapping_scheme == PRIO_FIFO) {
        if (priority >= num_queues) {
            throw cRuntimeError("priority_mapping_scheme == PRIO_FIFO && priority >= num_queues");
        }
        return priority;
    }
...
void V2PIFOPrioQueue::pushPacket(Packet *packet, cGate *gate)
{
    ...
    if (buffer != nullptr)
        ...
        if (should_insert_packet) {
            ...
            if (priority_mapping_scheme == PRIO_FIFO) {
                // reset the priority queue idx back to normal
                eth_header->setPrio_queue_idx(-1);
            }
...
bool V2PIFOPrioQueue::is_over_v2_threshold_full(b packet_length, Packet* packet, long on_the_way_packet_num, b on_the_way_packet_length) {
    if (priority_mapping_scheme == AIFO) {
        ...
        return false;
    throw cRuntimeError("is_over_v2_threshold_full::No priority_mapping_scheme!");
}
