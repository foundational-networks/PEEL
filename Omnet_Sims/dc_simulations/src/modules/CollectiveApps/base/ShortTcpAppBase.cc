...
simsignal_t ShortTcpAppBase::connectSignal = registerSignal("connect");

ShortTcpAppBase::~ShortTcpAppBase() {
    for (auto socket_pair: socket_map.getMap())
        delete socket_pair.second;
}

void ShortTcpAppBase::initialize(int stage)
{
...
void ShortTcpAppBase::handleMessageWhenUp(cMessage *msg)
{
    EV << "SEPEHR: Handling Message wShortTcpAppBaseTcpAppBase! msg is: " << msg->str() << endl;
...
void ShortTcpAppBase::connect(int dest_server_idx, int connect_port, unsigned long query_id)
{
    TcpSocket* socket;
    bool found = false;
...
//        emit(connectSignal, 1L);
    ...
    socket_dstserveridx_mapper.insert(std::pair<TcpSocket*, int>(socket, dest_server_idx));

}

unsigned long ShortTcpAppBase::get_query_id_for_socket(Packet *msg) {
    TcpSocket *socket = check_and_cast_nullable<TcpSocket*>(socket_map.findSocketFor(msg));
    if (!socket)
...
int ShortTcpAppBase::get_dst_server_idx_for_socket(Packet *msg) {
    TcpSocket *socket = check_and_cast_nullable<TcpSocket*>(socket_map.findSocketFor(msg));
    if (!socket)
        throw cRuntimeError("How is the socket not found2!");
    auto dst_idx_found = socket_dstserveridx_mapper.find(socket);
    if (dst_idx_found == socket_dstserveridx_mapper.end())
        throw cRuntimeError("How is the dst server idx not found!");
    int dst_idx = dst_idx_found->second;
    socket_query_mapper.erase(dst_idx_found->first);
    return dst_idx;
}

void ShortTcpAppBase::socketEstablished(TcpSocket *)
{
    // *redefine* to perform or schedule first sending
    EV_INFO << "connected\n";
}

void ShortTcpAppBase::close(int socket_id)
{
...
void ShortTcpAppBase::sendPacket(Packet *msg)
{
    TcpSocket *socket = check_and_cast_nullable<TcpSocket*>(socket_map.findSocketFor(msg));
    if (socket) {
...
void ShortTcpAppBase::refreshDisplay() const
{
    ApplicationBase::refreshDisplay();
//    getDisplayString().setTagArg("t", 0, TcpSocket::stateName(socket.getState()));
}

void ShortTcpAppBase::socketDataArrived(TcpSocket *, Packet *msg, bool)
{
...
void ShortTcpAppBase::socketPeerClosed(TcpSocket *socket_)
{
...
void ShortTcpAppBase::socketClosed(TcpSocket *socket)
{
    // *redefine* to start another session etc.
    EV_INFO << "connection closed\n";
}

void ShortTcpAppBase::socketFailure(TcpSocket *socket, int code)
{
