...
simsignal_t LongTcpAppBase::connectSignal = registerSignal("connect");

LongTcpAppBase::~LongTcpAppBase() {
    for (auto socket_pair: socket_map.getMap())
        delete socket_pair.second;
}

void LongTcpAppBase::initialize(int stage)
{
...
void LongTcpAppBase::handleMessageWhenUp(cMessage *msg)
{
    EV << "SEPEHR: Handling Message LongTcpAppBase! msg is: " << msg->str() << endl;
...
void LongTcpAppBase::connect(int dest_server_idx, int connect_port)
{
    TcpSocket* socket;
//    bool found = false;
...
void LongTcpAppBase::socketEstablished(TcpSocket *)
{
    // *redefine* to perform or schedule first sending
    EV_INFO << "connected\n";
}

void LongTcpAppBase::close(int socket_id)
{
...
void LongTcpAppBase::sendPacket(Packet *msg)
{
    TcpSocket *socket = check_and_cast_nullable<TcpSocket*>(socket_map.findSocketFor(msg));
    if (socket) {
...
void LongTcpAppBase::refreshDisplay() const
{
    ApplicationBase::refreshDisplay();
//    getDisplayString().setTagArg("t", 0, TcpSocket::stateName(socket.getState()));
}

void LongTcpAppBase::dataArrived(Packet *msg)
{
...
void LongTcpAppBase::socketDataArrived(TcpSocket *, Packet *msg, bool)
{
    dataArrived(msg);
}

void LongTcpAppBase::socketPeerClosed(TcpSocket *socket_)
{
...
void LongTcpAppBase::socketClosed(TcpSocket *socket)
{
    // *redefine* to start another session etc.
    EV_INFO << "connection closed\n";
}

void LongTcpAppBase::socketFailure(TcpSocket *socket, int code)
{
