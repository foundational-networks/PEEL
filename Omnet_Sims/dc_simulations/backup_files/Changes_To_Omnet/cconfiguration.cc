...
// The path for this file is: omnetpp(.ver)/src/sim/cconfiguration.cc
...
bool cConfiguration::parseBool(const char *s, const char *defaultValue, bool fallbackValue)
{
    ...
    if (strcmp(s, "yes") == 0 || strcmp(s, "true") == 0)
    	...
    	size_t pos = 0;
    	std::string token;
    	if (m_str.find(delimiter) != std::string::npos) {
    		pos = m_str.find(delimiter);
    		token = m_str.substr(0, pos);
    		m_str.erase(0, pos + delimiter.length());
    		if (strcmp(token.c_str(), m_str.c_str()) == 0) {
    			return true;
    		} 
    		else {
	    		return false;
