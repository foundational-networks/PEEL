...
for file_name in onlyfiles:
    ...
    if ('_ft_' in file_name):
      topology_info = file_name.split('_ft_')[0]
      topology_info_list = file_name.split('_')
      ft_k = topology_info_list[0]
      topology_type = Fattree
    elif ('_ls_' in file_name):
      ...
      raise Exception("Unknown topology info")
    
    if topology_type is None:
       raise Exception("topology_type is None")
    ...
    frag_perc = None
    if ('_frag' in file_name):
      frag_perc = (file_name.split('_frag')[0]).split('_')[-1]
    ...
    for rep_num in range(REP_NUM):
        if topology_type == Leafspine:
          ...
          without_format_input_file_name = '{}_ft_{}_mlapp_{}_bursty_{}_bg_' \
                                    '{}_reqpburst_{}_bgintermult_{}_burstyintermult_{}_mlqueryintermult_' \
                                      '{}_rep_{}_incastfsize_{}_mlscale_{}_mlfsize_{}_gpus_{}_nv' \
              .format(ft_k,
                num_ml_apps, num_bursty_apps, num_bg_apps, num_req_per_burst,
          ...
          without_format_output_file_name = '{}_ft_{}_mlapp_{}_bursty_{}_bg_' \
                                    '{}_reqpburst_{}_bgintermult_{}_burstyintermult_{}_mlqueryintermult_' \
                                      '{}_incastfsize_{}_mlscale_{}_mlfsize_{}_gpus_{}_nv' \
              .format(ft_k,
                num_ml_apps, num_bursty_apps, num_bg_apps, num_req_per_burst,
        ...
        if (frag_perc is not None):
          without_format_input_file_name += '_{}_frag'.format(frag_perc)
          without_format_output_file_name += '_{}_frag'.format(frag_perc)
        ...
        output_dir_name = 'CONTROLLER_DELAY/'
        command = "scavetool x --type v --filter \"module(**.server[*].app[*]) AND " \
                  "\\\"controllerDelay:vector\\\"\" -o {} -F CSV-S {}\n".format(
            OUTPUT_FILE_DIRECTORY + output_dir_name + output_file_name, vector_file_name)
        print(command)
        f.write('echo \"{}\"\n'.format(output_dir_name))
        f.write(command)
        ...
        output_dir_name = 'AUTOCCL_CHOSEN_ALG/'
        command = "scavetool x --type v --filter \"module(**.server[*].app[*]) AND " \
                  "\\\"autocclChosenCollectiveAlg:vector\\\"\" -o {} -F CSV-S {}\n".format(
            OUTPUT_FILE_DIRECTORY + output_dir_name + output_file_name, vector_file_name)
        print(command)
        f.write('echo \"{}\"\n'.format(output_dir_name))
        f.write(command)

        output_dir_name = 'AUTOCCL_CHOSEN_COLLECTIVE/'
        command = "scavetool x --type v --filter \"module(**.server[*].app[*]) AND " \
                  "\\\"autocclChosenCollective:vector\\\"\" -o {} -F CSV-S {}\n".format(
            OUTPUT_FILE_DIRECTORY + output_dir_name + output_file_name, vector_file_name)
        print(command)
        f.write('echo \"{}\"\n'.format(output_dir_name))
        f.write(command)
        ...
        output_dir_name = 'POPPED_DATA_PKT_NUM/'
        command = "scavetool x --type s --filter \"module(**.**.eth[*].mac.queue*) AND " \
                  "\\\"lightPoppedDataPktNum\\\"\" -o {} -F CSV-S {}\n".format(OUTPUT_FILE_DIRECTORY+output_dir_name+output_file_name, scalar_file_name)
        print(command)
        f.write('echo \"{}\"\n'.format(output_dir_name))
        f.write(command)
        
        output_dir_name = 'LIGHT_LAST_DATA_POPPED_TIME/'
        command = "scavetool x --type s --filter \"module(**.**.eth[*].mac.queue*) AND " \
                  "\\\"lightLastPktPopTime\\\"\" -o {} -F CSV-S {}\n".format(OUTPUT_FILE_DIRECTORY+output_dir_name+output_file_name, scalar_file_name)
        print(command)
        f.write('echo \"{}\"\n'.format(output_dir_name))
        f.write(command)

        output_dir_name = 'AGG_POPPED_BYTES/'
        command = "scavetool x --type v --filter \"module(**.**.eth[*].mac.queue*) AND " \
                  "\\\"poppedBytes:*\\\"\" -o {} -F CSV-S {}\n".format(
            OUTPUT_FILE_DIRECTORY + output_dir_name + output_file_name, vector_file_name)
        print(command)
        f.write('echo \"{}\"\n'.format(output_dir_name))
        f.write(command)
        ...
        output_dir_name = 'LIGHT_TREE_COUNTER/'
        command = "scavetool x --type s --filter \"module(**.**.relayUnit) AND " \
                  "\\\"lightTreeCounter\\\"\" -o {} -F CSV-S {}\n".format(OUTPUT_FILE_DIRECTORY+output_dir_name+output_file_name, scalar_file_name)
        print(command)
        f.write('echo \"{}\"\n'.format(output_dir_name))
        f.write(command)
        
        output_dir_name = 'MAX_SIMULTANEOUS_TREES/'
        command = "scavetool x --type s --filter \"module(**.**.relayUnit) AND " \
                  "\\\"maxSimultaneousTrees\\\"\" -o {} -F CSV-S {}\n".format(OUTPUT_FILE_DIRECTORY+output_dir_name+output_file_name, scalar_file_name)
        print(command)
        f.write('echo \"{}\"\n'.format(output_dir_name))
        f.write(command)
        ...
        output_dir_name = 'LIGHT_DATA_PKT_SENT_NUM/'
        command = "scavetool x --type s --filter \"module(**.**.eth[*].mac) AND " \
                  "\\\"lightSentDataPktNum\\\"\" -o {} -F CSV-S {}\n".format(OUTPUT_FILE_DIRECTORY+output_dir_name+output_file_name, scalar_file_name)
        print(command)
        f.write('echo \"{}\"\n'.format(output_dir_name))
        f.write(command)

        output_dir_name = 'LIGHT_DATA_PKT_RCVD_NUM/'
        command = "scavetool x --type s --filter \"module(**.**.eth[*].mac) AND " \
                  "\\\"lightRcvdDataPktNum\\\"\" -o {} -F CSV-S {}\n".format(OUTPUT_FILE_DIRECTORY+output_dir_name+output_file_name, scalar_file_name)
        print(command)
        f.write('echo \"{}\"\n'.format(output_dir_name))
        f.write(command)

        output_dir_name = 'LIGHT_OOO_DATA_COUNT/'
        command = "scavetool x --type s --filter \"module(**.**.eth[*].mac) AND " \
                  "\\\"lightOOOdatacount\\\"\" -o {} -F CSV-S {}\n".format(OUTPUT_FILE_DIRECTORY+output_dir_name+output_file_name, scalar_file_name)
        print(command)
        f.write('echo \"{}\"\n'.format(output_dir_name))
        f.write(command)
