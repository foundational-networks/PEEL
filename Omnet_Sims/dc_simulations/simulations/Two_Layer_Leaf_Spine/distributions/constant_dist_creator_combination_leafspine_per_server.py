...
def get_allgather_participants_one_job_per_gpu(NUM_FLOWS_PER_ML_QUERY, ML_FLOW_SIZE, \
    query_time, server_to_available_time_dict, query_type):
    ...
    start_server_idx = random.randint(0, NUM_AGGS * num_servers_per_tor - 1)

    # first check if we have enough tors available
    enough_servers_available = False
    available_servers_num = 0
    required_servers = int(NUM_FLOWS_PER_ML_QUERY / NUM_GPUS)
    i = 0
    server_idx = start_server_idx
    while (available_servers_num < required_servers and i < NUM_AGGS * num_servers_per_tor):
        if server_idx not in server_to_available_time_dict:
            server_to_available_time_dict[server_idx] = 0
        if server_to_available_time_dict[server_idx] <= query_time:
            available_servers_num += 1
        if (available_servers_num == required_servers):
            enough_servers_available = True
        server_idx += 1
        server_idx %= (NUM_AGGS * num_servers_per_tor)
        i += 1

    if (not enough_servers_available):
        query_time = earliest_time_for_X_tors(server_to_available_time_dict, required_servers)

    chosen_servers = []
    server_idx = start_server_idx

    print(query_type)
    random_processing_overhead = 1
    if ('allgather' in query_type or 'alltoallv' in query_type):
        random_processing_overhead = NUM_FLOWS_PER_ML_QUERY
        # random_processing_overhead = random.randint(1, NUM_FLOWS_PER_ML_QUERY)
        
    elif ('allreduce' in query_type):
        random_processing_overhead = NUM_FLOWS_PER_ML_QUERY / num_gpus_under_tor
        # random_processing_overhead = random.randint(1, int(NUM_FLOWS_PER_ML_QUERY / num_gpus_under_tor))
    # random_processing_overhead =  1 + random.uniform(0, 0.25)
    processing_time = random_processing_overhead * ML_FLOW_SIZE * 8 / (100 * 10**9)

    i = 0
    while (len(chosen_servers) < required_servers):
        if server_to_available_time_dict[server_idx] <= query_time:
            chosen_servers.append(server_idx)
        else:
            print(f"Skipping server {server_idx}")
        server_idx += 1
        server_idx %= (NUM_AGGS * num_servers_per_tor)
        i += 1
        if (i == (NUM_AGGS * num_servers_per_tor) and \
            len(chosen_servers) < required_servers):
            raise Exception("All tors have been visited!")

    index_of_dest_servers = []
    index_of_dest_gpus = []
    for chosen_server in chosen_servers:
            server_to_available_time_dict[chosen_server] = query_time + processing_time
            possible_dsts = [chosen_server for _ in range(NUM_GPUS)]
            added_gpus = [m for m in range(NUM_GPUS)]
            index_of_dest_servers += possible_dsts
            index_of_dest_gpus += added_gpus
...
def fill_tables(rep_num, collective_alg_percentage_dict, 
    ...
    total_num_flows = 0
    server_to_available_time_dict = dict()
    for ml_query_idx in range(NUM_ML_QUERIES):
        if one_job_per_gpu:
            participants, participant_gpus, first_available_query_time = \
                get_allgather_participants_one_job_per_gpu(NUM_FLOWS_PER_ML_QUERY, \
                    ML_FLOW_SIZE, query_times[ml_query_idx], 
                    server_to_available_time_dict, 
                    query_types[ml_query_idx])
...
TRACE_FILE_NAME = "collectives_algs_dist_huawei.csv"
