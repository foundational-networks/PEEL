...
def get_allgather_participants_fragment(NUM_FLOWS_PER_ML_QUERY, ML_FLOW_SIZE, query_time, server_gpu_to_available_time_dict, fragment_ratio):
    ...
    if (len(server_gpu_to_available_time_dict) == 0):
        for server_idx in range(NUM_AGGS * num_servers_per_tor):
    ...
    if (NUM_FLOWS_PER_ML_QUERY > (num_gpus_under_tor * NUM_AGGS * fragment_ratio)):
        print(NUM_FLOWS_PER_ML_QUERY)
        print(num_gpus_under_tor * NUM_AGGS * fragment_ratio)
    ...
    while_counter = 0
    while (scale > 0):
        for relative_tor_idx in range(NUM_AGGS):

            if (scale == 0):
                break

            tor_idx = (start_tor_idx + relative_tor_idx) % (NUM_AGGS)

            # get num available nodes
            available_nodes = []
            start_server_idx = tor_idx * num_servers_per_tor
            end_server_idx = (tor_idx+1) * num_servers_per_tor
            for server_idx in range(start_server_idx, end_server_idx):
                for gpu_idx in range(NUM_GPUS):
                    if (query_time >= server_gpu_to_available_time_dict[(server_idx, gpu_idx)]):
                        available_nodes.append((server_idx, gpu_idx))
            busy_node_num = num_gpus_under_tor - len(available_nodes)
            node_num_to_remove_for_fragmentation = int(num_gpus_under_tor * (1 - fragment_ratio))
            # if we already have enough busy nodes to fragment, don't fragment further
            node_num_to_remove_for_fragmentation = max(0, node_num_to_remove_for_fragmentation-busy_node_num)

            if (node_num_to_remove_for_fragmentation > len(available_nodes)):
                raise Exception("How is the nodes to remove due to fragmentation larger than available nodes!")

            # Randomly select node_num_to_remove_for_fragmentation elements to remove
            to_remove = random.sample(available_nodes, node_num_to_remove_for_fragmentation)

            # Remove them
            for item in to_remove:
                available_nodes.remove(item)
            
            for node in available_nodes:
                index_of_dest_servers.append(node[0])
                index_of_dest_gpus.append(node[1])
                scale -= 1
                if (scale == 0):
                    break
