...
failure_set_file_path = "../distributions/web_allgather_failed_links_1.000000_flowmult_6.666000_intermult_64_flows_per_ml_query_8000000_flow_size_8_gpus_10_failperc_0.txt"

match = re.search(r'(\d+)_failperc', failure_set_file_path)
if match:
    # Extract the matched integer
    fail_perc = int(match.group(1))
    print("Expected failure percentage is:", fail_perc)
else:
    raise Exception("Failure percentage not found!")

failure_set = set()
print(f'Reading failure file: {failure_set_file_path}')
with open(failure_set_file_path, "r") as file:
    for line in file:
        # Strip newline or extra whitespace and add to the set
        failure_set.add(line.strip())
print(f'failed links set: {failure_set}')
print(f'Failure percentage: {(len(failure_set)/2)/(NUM_AGGS * NUM_SPINES)*100}%')
...
for spine_idx in range(NUM_SPINES):
    file_name = f'spine[{spine_idx}]_{fail_perc}_failperc.txt'
    ...
    for spine_port_num in range(NUM_AGGS):
        ...
        # check if the link has not failed
        if (f'l{spine_port_num}sp{spine_idx}' not in failure_set and f'sp{spine_idx}l{spine_port_num}' not in failure_set):
...
for src_agg_idx in range(NUM_AGGS):
    file_name = f'agg[{src_agg_idx}]_{fail_perc}_failperc.txt'
    file = open(file_name, 'w')
        ...
        for gpu_idx in range(len(dst_rack.gpus)):
            ...
            if src_rack.rack_id == dst_rack.rack_id:
                ...
                for spine_num in range(NUM_SPINES):
                    if (f'l{src_agg_idx}sp{spine_num}' not in failure_set and \
                        f'sp{spine_num}l{src_agg_idx}' not in failure_set and \
                            f'l{dest_agg_idx}sp{spine_num}' not in failure_set \
                                and f'sp{spine_num}l{dest_agg_idx}' not in failure_set):
